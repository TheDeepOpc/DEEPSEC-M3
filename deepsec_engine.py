#!/usr/bin/env python3
"""
DeepSec Engine - Autonomous Offensive Security Core
Author: TheDeepOpc

Production-focused autonomous security orchestration runtime
 Recon Operations | Validation Runs | Security Engineering

CURRENT CHARACTERISTICS:
 Unified neutral terminal presentation for professional operations
 Removed duplicate classes (PythonEnvironmentManager, CVEIntelligenceManager)
 Structured terminal presentation layer with OpsRenderCore
...
Architecture: Two-module runtime (deepsec_engine.py + deepsec_bridge.py)
Framework: FastMCP runtime for autonomous agent transport
"""
...
import shlex
import shutil
import signal as sigctl
import socket as sockio
import subprocess
import sys
import threading
import time
...
# Compatibility aliases preserve import stability for existing integrations.
psutil = sysprobe
signal = sigctl
requests = netreq
re = rex
...
try:
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('deepsec.log')
        ]
    )
...
# API Configuration
API_PORT = int(os.environ.get('DEEPSEC_PORT', 8888))
API_HOST = os.environ.get('DEEPSEC_HOST', '127.0.0.1')
...
ollama_runtime = DeepSecOllamaRuntime(
...
def build_ai_runtime_metadata(task_name: str, result: Dict[str, Any]) -> Dict[str, Any]:
    """Create consistent metadata for AI runtime usage across endpoints."""
    return {
        "task": task_name,
        "provider": "ollama",
        "base_url": OLLAMA_BASE_URL,
...
class NovaRenderCore:
    """Structured terminal output formatting with consistent ANSI styling."""

    # Base palette for operational terminal output.
    COLORS = {
        ...
        'BOLD': '\033[1m',
        'DIM': '\033[2m',
        # Backward-compatible color aliases retained for runtime compatibility.
    ...
    def create_banner() -> str:
        """Create the DeepSec operations banner."""
        # Build a distinct DeepSec operations-console style banner
        border_color = NovaRenderCore.COLORS['PRIMARY_BORDER']
        accent = NovaRenderCore.COLORS['ACCENT_LINE']
        gradient = NovaRenderCore.COLORS['ACCENT_GRADIENT']
        RESET = NovaRenderCore.COLORS['RESET']
        BOLD = NovaRenderCore.COLORS['BOLD']
...
{RESET}
{border_color}
 {NovaRenderCore.COLORS['BRIGHT_WHITE']}DEEPSEC OPERATIONS CONSOLE{border_color}                                             

 {accent}MODE      : ADAPTIVE EXECUTION SHELL{border_color}                                  
 {gradient}STACK     : RECON, VALIDATION, REPORTING{border_color}                               
 {accent}FOCUS     : SIGNAL CLARITY AND OPERATOR CONTROL{border_color}                         
 {gradient}SESSION   : RUNTIME INTELLIGENCE ORCHESTRATOR{border_color}                           
{RESET}

{NovaRenderCore.COLORS['TERMINAL_GRAY']}[INFO] Server starting on {API_HOST}:{API_PORT}
[INFO] Module mesh online | Tactical planning engine active
[INFO] DeepSec console profile loaded successfully{RESET}
"""
        return banner

    @staticmethod
    def create_progress_bar(current: int, total: int, width: int = 50, tool: str = "") -> str:
        """Create a structured progress bar with cyberpunk styling"""
        if total == 0:
            percentage = 0
        else:
        ...
        border = NovaRenderCore.COLORS['PRIMARY_BORDER']
        fill_col = NovaRenderCore.COLORS['ACCENT_LINE']
        return f"""
{border} {tool} {'' * (width - len(tool) - 4)}
 {fill_col}{bar}{border}  {percentage:6.1f}%
{'' * (width + 10)}{NovaRenderCore.COLORS['RESET']}"""

    @staticmethod
    def render_progress_bar(progress: float, width: int = 40, style: str = 'cyber',
                          label: str = "", eta: float = 0, speed: str = "") -> str:
        """Render a structured progress bar with multiple styles"""
        ...
        if style == 'cyber':
            filled_char = ''
            empty_char = ''
            bar_color = NovaRenderCore.COLORS['ACCENT_LINE']
            progress_color = NovaRenderCore.COLORS['PRIMARY_BORDER']
        elif style == 'matrix':
            filled_char = ''
            empty_char = ''
            bar_color = NovaRenderCore.COLORS['ACCENT_LINE']
            progress_color = NovaRenderCore.COLORS['ACCENT_GRADIENT']
        elif style == 'neon':
            filled_char = ''
            empty_char = ''
            bar_color = NovaRenderCore.COLORS['PRIMARY_BORDER']
            progress_color = NovaRenderCore.COLORS['CYBER_ORANGE']
        else:  # default
            filled_char = ''
            empty_char = ''
            bar_color = NovaRenderCore.COLORS['ACCENT_LINE']
        ...
        empty_part = NovaRenderCore.COLORS['TERMINAL_GRAY'] + empty_char * empty_width
        ...
        bar_display = f"[{filled_part}{empty_part}{NovaRenderCore.COLORS['RESET']}] {progress_color}{percentage}{NovaRenderCore.COLORS['RESET']}"
    ...
    def create_live_dashboard(processes: Dict[int, Dict[str, Any]]) -> str:
        ...
        if not processes:
            return f"""
{NovaRenderCore.COLORS['PRIMARY_BORDER']}
 {NovaRenderCore.COLORS['ACCENT_LINE']} DEEPSEC LIVE DASHBOARD{NovaRenderCore.COLORS['PRIMARY_BORDER']}                                           

 {NovaRenderCore.COLORS['TERMINAL_GRAY']}No active processes currently running{NovaRenderCore.COLORS['PRIMARY_BORDER']}                                    
{NovaRenderCore.COLORS['RESET']}
"""

        dashboard_lines = [
            f"{NovaRenderCore.COLORS['PRIMARY_BORDER']}",
            f" {NovaRenderCore.COLORS['ACCENT_LINE']} DEEPSEC LIVE DASHBOARD{NovaRenderCore.COLORS['PRIMARY_BORDER']}                                           ",
            f""
        ]
        ...
        for pid, proc_info in processes.items():
            ...
            status_color = NovaRenderCore.COLORS['ACCENT_LINE'] if status == 'running' else NovaRenderCore.COLORS['HACKER_RED']

            dashboard_lines.append(
                f" {NovaRenderCore.COLORS['CYBER_ORANGE']}PID {pid}{NovaRenderCore.COLORS['PRIMARY_BORDER']} | {status_color}{status}{NovaRenderCore.COLORS['PRIMARY_BORDER']} | {NovaRenderCore.COLORS['BRIGHT_WHITE']}{command}{NovaRenderCore.COLORS['PRIMARY_BORDER']} "
            )

        dashboard_lines.append(f"{NovaRenderCore.COLORS['RESET']}")

        return "\n".join(dashboard_lines)

    @staticmethod
    def format_vulnerability_card(vuln_data: Dict[str, Any]) -> str:
        """Format vulnerability as a structured card"""
        ...
        severity_colors = {
            'CRITICAL': NovaRenderCore.COLORS['VULN_CRITICAL'],
            'HIGH': NovaRenderCore.COLORS['HACKER_RED'],
            'MEDIUM': NovaRenderCore.COLORS['ACCENT_GRADIENT'],
            'LOW': NovaRenderCore.COLORS['CYBER_ORANGE'],
            'INFO': NovaRenderCore.COLORS['TERMINAL_GRAY']
        }

        color = severity_colors.get(severity, NovaRenderCore.COLORS['TERMINAL_GRAY'])

        return f"""
{color}  VULNERABILITY DETECTED 
 {NovaRenderCore.COLORS['BRIGHT_WHITE']}{name:<60}{color} 
 {NovaRenderCore.COLORS['TERMINAL_GRAY']}Severity: {color}{severity:<52}{color} 
 {NovaRenderCore.COLORS['TERMINAL_GRAY']}{description[:58]:<58}{color} 
{NovaRenderCore.COLORS['RESET']}"""
    ...
    def format_error_card(error_type: str, tool_name: str, error_message: str, recovery_action: str = "") -> str:
        ...
        error_colors = {
            'CRITICAL': NovaRenderCore.COLORS['VULN_CRITICAL'],
            'ERROR': NovaRenderCore.COLORS['TOOL_FAILED'],
            'TIMEOUT': NovaRenderCore.COLORS['TOOL_TIMEOUT'],
            'RECOVERY': NovaRenderCore.COLORS['TOOL_RECOVERY'],
            'WARNING': NovaRenderCore.COLORS['WARNING']
        }

        color = error_colors.get(error_type.upper(), NovaRenderCore.COLORS['ERROR'])

        card = f"""
{color}  ERROR DETECTED {NovaRenderCore.COLORS['RESET']}
{color} {NovaRenderCore.COLORS['BRIGHT_WHITE']}Tool: {tool_name:<55}{color} {NovaRenderCore.COLORS['RESET']}
{color} {NovaRenderCore.COLORS['BRIGHT_WHITE']}Type: {error_type:<55}{color} {NovaRenderCore.COLORS['RESET']}
{color} {NovaRenderCore.COLORS['BRIGHT_WHITE']}Error: {error_message[:53]:<53}{color} {NovaRenderCore.COLORS['RESET']}"""

        if recovery_action:
            card += f"""
{color} {NovaRenderCore.COLORS['TOOL_RECOVERY']}Recovery: {recovery_action[:50]:<50}{color} {NovaRenderCore.COLORS['RESET']}"""

        card += f"""
{color}{NovaRenderCore.COLORS['RESET']}"""
    ...
    def format_tool_status(tool_name: str, status: str, target: str = "", progress: float = 0.0) -> str:
        ...
        status_colors = {
            'RUNNING': NovaRenderCore.COLORS['TOOL_RUNNING'],
            'SUCCESS': NovaRenderCore.COLORS['TOOL_SUCCESS'],
            'FAILED': NovaRenderCore.COLORS['TOOL_FAILED'],
            'TIMEOUT': NovaRenderCore.COLORS['TOOL_TIMEOUT'],
            'RECOVERY': NovaRenderCore.COLORS['TOOL_RECOVERY']
        }

        color = status_colors.get(status.upper(), NovaRenderCore.COLORS['INFO'])
        ...
        if progress > 0:
            ...
            empty = 20 - filled
            progress_bar = f" [{NovaRenderCore.COLORS['PROGRESS_BAR']}{'' * filled}{NovaRenderCore.COLORS['PROGRESS_EMPTY']}{'' * empty}{NovaRenderCore.COLORS['RESET']}] {progress*100:.1f}%"

        return f"{color} {tool_name.upper()}{NovaRenderCore.COLORS['RESET']} | {color}{status}{NovaRenderCore.COLORS['RESET']} | {NovaRenderCore.COLORS['BRIGHT_WHITE']}{target}{NovaRenderCore.COLORS['RESET']}{progress_bar}"

    @staticmethod
    def format_highlighted_text(text: str, highlight_type: str = "RED") -> str:
        """Format text with highlighting background"""
        highlight_colors = {
            'RED': NovaRenderCore.COLORS['HIGHLIGHT_RED'],
            'YELLOW': NovaRenderCore.COLORS['HIGHLIGHT_YELLOW'],
            'GREEN': NovaRenderCore.COLORS['HIGHLIGHT_GREEN'],
            'BLUE': NovaRenderCore.COLORS['HIGHLIGHT_BLUE'],
            'PURPLE': NovaRenderCore.COLORS['HIGHLIGHT_PURPLE']
        }

        color = highlight_colors.get(highlight_type.upper(), NovaRenderCore.COLORS['HIGHLIGHT_RED'])
        return f"{color} {text} {NovaRenderCore.COLORS['RESET']}"
    ...
    def format_vulnerability_severity(severity: str, count: int = 0) -> str:
        ...
        severity_colors = {
            'CRITICAL': NovaRenderCore.COLORS['VULN_CRITICAL'],
            'HIGH': NovaRenderCore.COLORS['VULN_HIGH'],
            'MEDIUM': NovaRenderCore.COLORS['VULN_MEDIUM'],
            'LOW': NovaRenderCore.COLORS['VULN_LOW'],
            'INFO': NovaRenderCore.COLORS['VULN_INFO']
        }

        color = severity_colors.get(severity.upper(), NovaRenderCore.COLORS['INFO'])
        count_text = f" ({count})" if count > 0 else ""

        return f"{color}{severity.upper()}{count_text}{NovaRenderCore.COLORS['RESET']}"

    @staticmethod
    def create_section_header(title: str, icon: str = "", color: str = "FIRE_RED") -> str:
        """Create a section header with reddish styling"""
        header_color = NovaRenderCore.COLORS.get(color, NovaRenderCore.COLORS['FIRE_RED'])

        return f"""
{header_color}{'' * 70}{NovaRenderCore.COLORS['RESET']}
{header_color}{icon} {title.upper()}{NovaRenderCore.COLORS['RESET']}
{header_color}{'' * 70}{NovaRenderCore.COLORS['RESET']}"""
    ...
    def format_command_execution(command: str, status: str, duration: float = 0.0) -> str:
        ...
        status_colors = {
            'STARTING': NovaRenderCore.COLORS['INFO'],
            'RUNNING': NovaRenderCore.COLORS['TOOL_RUNNING'],
            'SUCCESS': NovaRenderCore.COLORS['TOOL_SUCCESS'],
            'FAILED': NovaRenderCore.COLORS['TOOL_FAILED'],
            'TIMEOUT': NovaRenderCore.COLORS['TOOL_TIMEOUT']
        ...
        return f"{color} {command[:60]}{'...' if len(command) > 60 else ''} | {status.upper()}{duration_text}{NovaRenderCore.COLORS['RESET']}"
...
class TacticalFlowPlanner:
    ...
    def _optimize_ghidra_params(self, profile: TargetProfile, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        params["project_name"] = f"deepsec_{binary_name}"
...
recon_planner = TacticalFlowPlanner()
...
class ResilienceEngine:
    ...
    def handle_tool_failure(self, tool: str, error: Exception, context: Dict[str, Any]) -> RecoveryStrategy:
        ...
        logger.warning(f"{NovaRenderCore.format_error_card('RECOVERY', tool, error_message)}")
    ...
    def auto_adjust_parameters(self, tool: str, error_type: ErrorType, original_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"{NovaRenderCore.format_tool_status(tool, 'RECOVERY', adjustment_info)}")
    ...
    def escalate_to_human(self, context: ErrorContext, urgency: str = "medium") -> Dict[str, Any]:
        ...
        logger.error(f"{NovaRenderCore.format_error_card('CRITICAL', context.tool_name, context.error_message, 'HUMAN ESCALATION REQUIRED')}")
        logger.error(f"{NovaRenderCore.format_highlighted_text('ESCALATION DETAILS', 'RED')}")
...
continuity_manager = FallbackContinuity()
...
# All color references consolidated to NovaRenderCore.COLORS for consistency
    BG_GREEN = '\033[42m'
    BG_YELLOW = '\033[43m'
    BG_BLUE = '\033[44m'
    BG_MAGENTA = '\033[45m'
...
class PythonEnvironmentManager:
    ...
    def __init__(self, base_dir: str = "/tmp/deepsec_envs"):
...
class CVEIntelligenceManager:
    ...
    @staticmethod
    def create_banner():
        """Reuse unified NovaRenderCore banner (legacy hook)."""
        return NovaRenderCore.create_banner()
    ...
    def render_progress_bar(progress: float, width: int = 40, style: str = 'cyber',
        ...
        bar = f"{progress_color}[{filled_part}{empty_part}{NovaRenderCore.COLORS['RESET']}{progress_color}] {percentage}{eta_str}{speed_str}{NovaRenderCore.COLORS['RESET']}"

        if label:
            return f"{NovaRenderCore.COLORS['BOLD']}{label}{NovaRenderCore.COLORS['RESET']} {bar}"
        return bar

    @staticmethod
    def render_vulnerability_card(vuln_data: Dict[str, Any]) -> str:
        """Render vulnerability as a structured card with severity indicators"""
        ...
        # Get severity color
        severity_color = NovaRenderCore.COLORS['HACKER_RED'] if severity == 'critical' else NovaRenderCore.COLORS['HACKER_RED'] if severity == 'high' else NovaRenderCore.COLORS['CYBER_ORANGE'] if severity == 'medium' else NovaRenderCore.COLORS['CYBER_ORANGE'] if severity == 'low' else NovaRenderCore.COLORS['NEON_BLUE']
        ...
        card = f"""
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {severity_color}{severity_badge}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['BOLD']}{title[:60]}{NovaRenderCore.COLORS['RESET']}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['NEON_BLUE']} Target:{NovaRenderCore.COLORS['RESET']} {url[:65]}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['CYBER_ORANGE']} CVSS:{NovaRenderCore.COLORS['RESET']} {cvss}/10.0
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['CYBER_ORANGE']} Description:{NovaRenderCore.COLORS['RESET']}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}   {description[:70]}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}
"""
        return card
    ...
    def create_live_dashboard(processes: Dict[int, Dict[str, Any]]) -> str:
        ...
        if not processes:
            return f"{NovaRenderCore.COLORS['TERMINAL_GRAY']} No active processes{NovaRenderCore.COLORS['RESET']}"

        dashboard = f"""
{NovaRenderCore.COLORS['MATRIX_GREEN']}{NovaRenderCore.COLORS['BOLD']}
...
"""
        ...
        for pid, proc_info in processes.items():
            ...
            # Status color coding
            status_colors = {
                'running': NovaRenderCore.COLORS['MATRIX_GREEN'],
                'paused': NovaRenderCore.COLORS['WARNING'],
                'terminated': NovaRenderCore.COLORS['ERROR'],
                'completed': NovaRenderCore.COLORS['NEON_BLUE']
            }
            status_color = status_colors.get(status, NovaRenderCore.COLORS['BRIGHT_WHITE'])

            # Create mini progress bar
            mini_bar = NovaRenderCore.render_progress_bar(
                progress, width=20, style='cyber', eta=eta
            )

            dashboard += f"""{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['NEON_BLUE']}PID {pid}{NovaRenderCore.COLORS['RESET']}  {status_color}{status.upper()}{NovaRenderCore.COLORS['RESET']}  {runtime:.1f}s  {command}...
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {mini_bar}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}
"""

        dashboard += f"{NovaRenderCore.COLORS['MATRIX_GREEN']}{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}"
    ...
    def format_tool_output(tool: str, output: str, success: bool = True) -> str:
        ...
        status_color = NovaRenderCore.COLORS['MATRIX_GREEN'] if success else NovaRenderCore.COLORS['HACKER_RED']

        # Format the output with structure
        formatted_output = f"""
{NovaRenderCore.COLORS['BOLD']} {tool_icon} {tool.upper()} OUTPUT {NovaRenderCore.COLORS['RESET']}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {status_color}{status_icon} Status: {'SUCCESS' if success else 'FAILED'}{NovaRenderCore.COLORS['RESET']}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}
"""
        ...
        for line in lines[:20]:  # Limit to first 20 lines for readability
            if line.strip():
                ...
                if any(keyword in line.lower() for keyword in ['error', 'failed', 'denied']):
                    formatted_output += f"{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['ERROR']}{line[:75]}{NovaRenderCore.COLORS['RESET']}\n"
                elif any(keyword in line.lower() for keyword in ['found', 'discovered', 'vulnerable']):
                    formatted_output += f"{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['MATRIX_GREEN']}{line[:75]}{NovaRenderCore.COLORS['RESET']}\n"
                elif any(keyword in line.lower() for keyword in ['warning', 'timeout']):
                    formatted_output += f"{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['WARNING']}{line[:75]}{NovaRenderCore.COLORS['RESET']}\n"
                else:
                    formatted_output += f"{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['BRIGHT_WHITE']}{line[:75]}{NovaRenderCore.COLORS['RESET']}\n"

        if len(lines) > 20:
            formatted_output += f"{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['TERMINAL_GRAY']}... ({len(lines) - 20} more lines truncated){NovaRenderCore.COLORS['RESET']}\n"

        formatted_output += f"{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}"
...
{NovaRenderCore.COLORS['MATRIX_GREEN']}{NovaRenderCore.COLORS['BOLD']}
                               SCAN SUMMARY REPORT                          
{NovaRenderCore.COLORS['RESET']}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['NEON_BLUE']} Target:{NovaRenderCore.COLORS['RESET']} {results.get('target', 'Unknown')[:60]}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['CYBER_ORANGE']}  Duration:{NovaRenderCore.COLORS['RESET']} {execution_time:.2f} seconds
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['WARNING']}  Tools Used:{NovaRenderCore.COLORS['RESET']} {len(tools_used)} tools
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['HACKER_RED']} Critical:{NovaRenderCore.COLORS['RESET']} {critical_vulns} vulnerabilities
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['ERROR']}  High:{NovaRenderCore.COLORS['RESET']} {high_vulns} vulnerabilities
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['MATRIX_GREEN']} Total Found:{NovaRenderCore.COLORS['RESET']} {total_vulns} vulnerabilities
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['ELECTRIC_PURPLE']} Tools:{NovaRenderCore.COLORS['RESET']} {', '.join(tools_used[:5])}{'...' if len(tools_used) > 5 else ''}
{NovaRenderCore.COLORS['MATRIX_GREEN']}{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}
"""
        return report
...
class ColoredFormatter(logging.Formatter):
    ...
    COLORS = {
        'DEBUG': NovaRenderCore.COLORS['DEBUG'],
        'INFO': NovaRenderCore.COLORS['SUCCESS'],
        'WARNING': NovaRenderCore.COLORS['WARNING'],
        'ERROR': NovaRenderCore.COLORS['ERROR'],
        'CRITICAL': NovaRenderCore.COLORS['CRITICAL']
    }
    ...
    def format(self, record):
        ...
        color = self.COLORS.get(record.levelname, NovaRenderCore.COLORS['BRIGHT_WHITE'])

        # Add color and emoji to the message
        record.msg = f"{color}{emoji} {record.msg}{NovaRenderCore.COLORS['RESET']}"
...
def setup_logging():
    ...
    console_handler.setFormatter(ColoredFormatter(
        "[ DeepSec AI] %(asctime)s [%(levelname)s] %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S"
    ))
...
class EnhancedCommandExecutor:
    ...
    def _show_progress(self, duration: float):
        ...
        if duration > 2:  # Show progress for commands taking more than 2 seconds
            progress_chars = NovaRenderCore.PROGRESS_STYLES['dots']
            start = time.time()
            i = 0
            while self.process and self.process.poll() is None:
                ...
                # Create structured progress bar using NovaRenderCore
                progress_bar = NovaRenderCore.render_progress_bar(
                    progress_fraction,
                    width=30,
                    style='cyber',
    ...
    def execute(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log enhanced final results with summary using NovaRenderCore
            ...
            status_icon = "" if success else ""
            status_color = NovaRenderCore.COLORS['MATRIX_GREEN'] if success else NovaRenderCore.COLORS['HACKER_RED']
            timeout_status = f" {NovaRenderCore.COLORS['WARNING']}[TIMEOUT]{NovaRenderCore.COLORS['RESET']}" if self.timed_out else ""

            # Create structured results summary
            results_summary = f"""
{NovaRenderCore.COLORS['MATRIX_GREEN']}{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {status_color} FINAL RESULTS {status_icon}{NovaRenderCore.COLORS['RESET']}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['NEON_BLUE']} Command:{NovaRenderCore.COLORS['RESET']} {self.command[:55]}{'...' if len(self.command) > 55 else ''}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['CYBER_ORANGE']}  Duration:{NovaRenderCore.COLORS['RESET']} {execution_time:.2f}s{timeout_status}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['WARNING']} Output Size:{NovaRenderCore.COLORS['RESET']} {output_size} bytes
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['ELECTRIC_PURPLE']} Exit Code:{NovaRenderCore.COLORS['RESET']} {self.return_code}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {status_color} Status:{NovaRenderCore.COLORS['RESET']} {'SUCCESS' if success else 'FAILED'} | Cached: Yes
{NovaRenderCore.COLORS['MATRIX_GREEN']}{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}
"""
...
def execute_command_with_recovery(tool_name: str, command: str, parameters: Dict[str, Any] = None,
    ...
    while attempt_count < max_attempts:
        ...
        try:
            ...
            recovery_strategy = resilience_engine.handle_tool_failure(tool_name, exception, context)
            ...
            if recovery_strategy.action == RecoveryAction.RETRY_WITH_BACKOFF:
                ...
                logger.info(f"{NovaRenderCore.format_tool_status(tool_name, 'RECOVERY', retry_info)}")
                time.sleep(actual_delay)
                continue

            elif recovery_strategy.action == RecoveryAction.RETRY_WITH_REDUCED_SCOPE:
                # Adjust parameters to reduce scope
                adjusted_params = resilience_engine.auto_adjust_parameters(
                    tool_name,
                    resilience_engine.classify_error(error_message, exception),
                    parameters
                )
                ...
                alternative_tool = resilience_engine.get_alternative_tool(tool_name, recovery_strategy.parameters)

                if alternative_tool:
                    switch_info = f'Switching to alternative: {alternative_tool}'
                    logger.info(f"{NovaRenderCore.format_tool_status(tool_name, 'RECOVERY', switch_info)}")
                ...
                error_type = resilience_engine.classify_error(error_message, exception)
                adjusted_params = resilience_engine.auto_adjust_parameters(tool_name, error_type, parameters)
                ...
                error_context = ErrorContext(
                    ...
                    timestamp=datetime.now(),
                    stack_trace="",
                    system_resources=resilience_engine._get_system_resources()
                )

                escalation_data = resilience_engine.escalate_to_human(
                    error_context,
                ...
                degraded_result = continuity_manager.handle_partial_failure(
                    operation,
                    result,
                    [tool_name]
                )
            ...
            if attempt_count >= max_attempts:
                ...
                escalation_data = resilience_engine.escalate_to_human(error_context, "high")
...
class FileOperationsManager:
    ...
    def __init__(self, base_dir: str = "/tmp/deepsec_files"):
        self.base_dir = Path(base_dir)
        self.base_dir.mkdir(exist_ok=True)
...
def web_panel_home():
    """Serve the production operations panel UI."""
...
def ai_chat():
    """General-purpose AI chat endpoint for the web panel and operators."""
    try:
        ...
        chat_result = ollama_runtime.generate_text(
            prompt=(
                "Operator request: provide concise actionable guidance for security operations. "
                "Focus on safe, authorized testing steps and practical execution details.\n\n"
                f"Message:\n{message}"
            ),
            system_prompt=(
                "You are DeepSec operational AI running in production. "
                "Return clear, practical, and security-focused responses without unnecessary filler."
            ),
            temperature=0.2,
            num_predict=900,
            context=context,
        ...
        if not chat_result.get("success"):
            runtime_meta = build_ai_runtime_metadata("chat", chat_result)
            if AI_ENFORCE_OLLAMA:
                return jsonify({
                    "success": False,
                    "error": "Ollama runtime failure during AI chat",
                    "details": chat_result.get("error", "Unknown error"),
                    "ai_runtime": runtime_meta,
                }), 503

            return jsonify({
                "success": True,
                "response": "AI runtime is unavailable. Local fallback mode is active.",
                "ai_runtime": runtime_meta,
                "timestamp": datetime.now().isoformat(),
            })
        ...
        return jsonify({
            ...
            "ai_runtime": build_ai_runtime_metadata("chat", chat_result),
            "timestamp": datetime.now().isoformat(),
        })
...
def health_check():
    ...
    for tool in all_tools:
        try:
            # Use cross-platform binary resolution (works on Windows and Unix-like systems).
...
def process_dashboard():
    """Get enhanced process dashboard with visual status using NovaRenderCore"""
    try:
        processes = ProcessManager.list_active_processes()
        current_time = time.time()

        # Create structured dashboard using NovaRenderCore
        dashboard_visual = NovaRenderCore.create_live_dashboard(processes)
...
def create_vulnerability_card():
    """Create a beautiful vulnerability card using NovaRenderCore"""
    try:
        ...
        card = NovaRenderCore.render_vulnerability_card(data)
...
def create_summary_report():
    """Create a structured summary report using NovaRenderCore"""
    try:
        data = request.get_json()
        if not data:
            return jsonify({"error": "No data provided"}), 400
...
def format_tool_output():
    """Format tool output using NovaRenderCore"""
    try:
        data = request.get_json()
        ...
        # Format tool output
        formatted_output = NovaRenderCore.format_tool_output(tool, output, success)
...
def analyze_target():
    ...
    try:
        ...
        profile = recon_planner.analyze_target(target)
        ...
        ai_result = ollama_runtime.generate_json(
            prompt=(
                "Analyze this target profile and return JSON with keys: "
                "risk_summary (string), priority_actions (list of strings), "
                "recommended_focus (list of strings), confidence_adjustment (number between -0.2 and 0.2)."
            ),
            system_prompt=(
                "You are DeepSec AI analyst. Return strict JSON only. "
                "Keep recommendations actionable for authorized security testing."
            ),
            context={
                "target": target,
        ...
        ai_runtime = build_ai_runtime_metadata("analyze-target", ai_result)
        if not ai_result.get("success") and AI_ENFORCE_OLLAMA:
            return jsonify({
                "success": False,
                "error": "Ollama runtime failure during target analysis",
                "details": ai_result.get("error", "Unknown error"),
                "ai_runtime": ai_runtime,
                "timestamp": datetime.now().isoformat(),
            }), 503

        ai_insights = ai_result.get("json", {}) if ai_result.get("success") else {}
...
def select_optimal_tools():
    ...
    try:
        ...
        ai_result = ollama_runtime.generate_json(
            prompt=(
                "Return JSON with keys: recommended_tools (list of strings), "
                "reasoning (string), and execution_notes (list of strings). "
        ...
        ai_runtime = build_ai_runtime_metadata("select-tools", ai_result)
        if not ai_result.get("success") and AI_ENFORCE_OLLAMA:
            return jsonify({
                "success": False,
                "error": "Ollama runtime failure during tool selection",
                "details": ai_result.get("error", "Unknown error"),
                "ai_runtime": ai_runtime,
                "timestamp": datetime.now().isoformat(),
            }), 503

        ai_plan = ai_result.get("json", {}) if ai_result.get("success") else {}
        ai_tools = ai_plan.get("recommended_tools", []) if isinstance(ai_plan, dict) else []
        if isinstance(ai_tools, list) and ai_tools:
            candidate_set = set(selected_tools)
            filtered_tools = [str(item).strip() for item in ai_tools if str(item).strip() in candidate_set]
            if filtered_tools:
                selected_tools = list(dict.fromkeys(filtered_tools))
...
def optimize_tool_parameters():
    ...
    try:
        ...
        # Optimize parameters
        optimized_params = recon_planner.optimize_parameters(tool, profile, context)

        ai_result = ollama_runtime.generate_json(
            prompt=(
                "Return JSON with keys: optimized_parameters (object), tuning_notes (list of strings), "
                "and safety_controls (list of strings). Keep parameters valid for the specified tool."
            ),
            system_prompt=(
                "You are DeepSec AI parameter optimizer. Return strict JSON only with practical values."
            ),
            context={
                "target": target,
                "tool": tool,
                "context": context,
                "current_parameters": optimized_params,
                "target_profile": profile.to_dict(),
            },
            temperature=0.1,
            num_predict=900,
        ...
        ai_runtime = build_ai_runtime_metadata("optimize-parameters", ai_result)
        if not ai_result.get("success") and AI_ENFORCE_OLLAMA:
            return jsonify({
                "success": False,
                "error": "Ollama runtime failure during parameter optimization",
                "details": ai_result.get("error", "Unknown error"),
                "ai_runtime": ai_runtime,
                "timestamp": datetime.now().isoformat(),
            }), 503

        ai_tuning = ai_result.get("json", {}) if ai_result.get("success") else {}
        ai_params = ai_tuning.get("optimized_parameters", {}) if isinstance(ai_tuning, dict) else {}
        if isinstance(ai_params, dict):
            optimized_params.update(ai_params)
...
def create_attack_chain():
    ...
    try:
        ...
        # Create attack chain
        attack_chain = recon_planner.create_attack_chain(profile, objective)

        ai_result = ollama_runtime.generate_json(
            prompt=(
                "Return JSON with keys: chain_summary (string), critical_steps (list of strings), "
                "risk_controls (list of strings), and expected_outcome (string)."
            ),
            system_prompt=(
                "You are DeepSec AI attack-chain reviewer. Return strict JSON and keep guidance concise."
            ),
            context={
                "target": target,
                "objective": objective,
        ...
        ai_runtime = build_ai_runtime_metadata("create-attack-chain", ai_result)
        if not ai_result.get("success") and AI_ENFORCE_OLLAMA:
            return jsonify({
                "success": False,
                "error": "Ollama runtime failure during attack chain generation",
                "details": ai_result.get("error", "Unknown error"),
                "ai_runtime": ai_runtime,
                "timestamp": datetime.now().isoformat(),
            }), 503

        ai_chain_review = ai_result.get("json", {}) if ai_result.get("success") else {}
        ...
        return jsonify({
            ...
            "target_profile": profile.to_dict(),
            "attack_chain": attack_chain.to_dict(),
            "ai_chain_review": ai_chain_review,
            "ai_runtime": ai_runtime,
            "timestamp": datetime.now().isoformat()
        })
...
def intelligent_smart_scan():
    ...
    try:
        ...
        valid_objectives = {"quick", "comprehensive", "stealth"}
        autonomous_mode = bool(data.get("autonomous_mode", True))
        objective_raw = str(data.get("objective", "") or "").strip().lower()
        objective_provided = objective_raw in valid_objectives
        objective = objective_raw if objective_provided else "comprehensive"

        requested_max_tools = data.get('max_tools', 0)
        try:
            requested_max_tools = int(requested_max_tools)
        except (TypeError, ValueError):
            requested_max_tools = 0

        # In autonomous mode, AI controls scan breadth unless explicitly overridden.
        if autonomous_mode and not bool(data.get("respect_max_tools", False)):
            requested_max_tools = 0
        ...
        auto_budget = requested_max_tools <= 0
        if auto_budget:
            max_tools = objective_defaults.get(objective, 24)
        else:
            max_tools = max(1, min(requested_max_tools, 30))

        # Backward-compatibility: older panel builds sent 5 by default.
        # Unless explicitly overridden, upgrade this legacy default to AI auto budget.
        if requested_max_tools == 5 and not bool(data.get("respect_max_tools", False)):
            max_tools = objective_defaults.get(objective, 24)
            auto_budget = True
        ...
        host_no_port = parsed_target.hostname or host_target.split(":")[0]
        port_hint = parsed_target.port
        ...
        def _shell_quote(value: str) -> str:
            ...
            return shlex.quote(text_value)

        safe_host_token = _shell_quote(host_no_port)
        ...
        try:
            dns_name, dns_aliases, dns_addresses = socket.gethostbyname_ex(host_no_port)
            preflight_recon["dns"] = {
                "resolved": True,
                "hostname": dns_name,
                "aliases": dns_aliases[:5],
                "addresses": dns_addresses[:10],
            }
        ...
        if shutil.which("ping"):
            ping_cmd = f"ping -n 1 {safe_host_token}" if os.name == "nt" else f"ping -c 1 {safe_host_token}"
            ping_result = execute_command(ping_cmd, use_cache=False)
            ping_stdout = str(ping_result.get("stdout", "") or "")
            preflight_recon["ping"] = {
                "command": ping_cmd,
                "success": bool(ping_result.get("success", False)),
                "latency_hint": ping_stdout[:220],
                "error": str(ping_result.get("stderr", "") or "")[:220],
            }

        if shutil.which("curl"):
            curl_cmd = f"curl -I -m 6 -sS {safe_web_token}"
            curl_result = execute_command(curl_cmd, use_cache=False)
            curl_head = str(curl_result.get("stdout", "") or "")
            preflight_recon["curl_head"] = {
                "command": curl_cmd,
                "success": bool(curl_result.get("success", False)),
                "response_excerpt": curl_head[:600],
                "error": str(curl_result.get("stderr", "") or "")[:220],
            }

        try:
            http_probe = requests.get(
                web_target,
                timeout=6,
                allow_redirects=True,
                headers={"User-Agent": "DeepSec-AI/6.0-Preflight"},
            )
            page_text = http_probe.text or ""
            page_title = ""
            content_type = str(http_probe.headers.get("Content-Type", "") or "")
            if "html" in content_type.lower() and page_text:
                soup = BeautifulSoup(page_text, "html.parser")
                if soup.title and soup.title.text:
                    page_title = soup.title.text.strip()[:140]

            preflight_recon["http_probe"] = {
                "success": True,
                "final_url": str(http_probe.url),
                "status_code": int(http_probe.status_code),
                "content_type": content_type[:120],
                "server": str(http_probe.headers.get("Server", "") or "")[:120],
                "title": page_title,
                "body_excerpt": page_text[:800],
            }
        ...
        http_probe_data = preflight_recon.get("http_probe", {})
        curl_data = preflight_recon.get("curl_head", {})
        page_title_lc = str(http_probe_data.get("title", "") or "").lower()
        body_excerpt_lc = str(http_probe_data.get("body_excerpt", "") or "").lower()
        content_type_lc = str(http_probe_data.get("content_type", "") or "").lower()

        preflight_recon["web_detected"] = bool(http_probe_data.get("success") or curl_data.get("success"))

        hints: List[str] = []
        if preflight_recon["web_detected"]:
            hints.append("reachable_web_target")
        if "application/json" in content_type_lc:
            hints.append("api_like_response")
        if any(k in page_title_lc or k in body_excerpt_lc for k in ["bank", "payment", "wallet", "checkout", "transfer", "swift", "card"]):
            hints.append("financial_keywords_detected")
        if any(k in page_title_lc or k in body_excerpt_lc for k in ["login", "signin", "auth", "account", "dashboard"]):
            hints.append("authentication_surface_detected")
        if any(k in content_type_lc for k in ["html", "javascript", "json", "xml"]):
            hints.append("application_layer_exposed")
        preflight_recon["target_hints"] = hints

        if "financial_keywords_detected" in hints:
            preflight_recon["target_kind"] = "financial_web_application"
        elif "api_like_response" in hints:
            preflight_recon["target_kind"] = "api_service"
        elif preflight_recon["web_detected"]:
            preflight_recon["target_kind"] = "web_application"
        elif preflight_recon.get("dns", {}).get("resolved"):
            preflight_recon["target_kind"] = "host_or_network_service"

        objective_auto_selected = False
        objective_decision_reason = "Operator objective accepted" if objective_provided else "Default objective applied"
        if autonomous_mode and not objective_provided:
            target_kind = preflight_recon.get("target_kind", "unknown")
            if target_kind in {"financial_web_application", "web_application", "api_service"}:
                objective = "comprehensive"
            elif target_kind == "host_or_network_service":
                objective = "quick"
            else:
                objective = "quick"
        ...
        autonomous_decision = {
            ...
            "objective": objective,
            "objective_auto_selected": objective_auto_selected,
        ...
        live_event_bus.emit(
            ...
            trace_id=trace_id,
            data={
                "target_kind": preflight_recon["target_kind"],
                "hints": preflight_recon["target_hints"],
                "status_code": http_probe_data.get("status_code"),
                "title": http_probe_data.get("title", ""),
            },
        )
        ...
        strategy_result = ollama_runtime.generate_json(
            prompt=(
                "Return JSON with keys: objective_override (string), max_tools (integer), "
            ...
            context={
                ...
                "preflight_recon": {
                    "target_kind": preflight_recon.get("target_kind"),
                    "target_hints": preflight_recon.get("target_hints", []),
                    "dns": preflight_recon.get("dns", {}),
                    "ping": preflight_recon.get("ping", {}),
                    "curl_head": preflight_recon.get("curl_head", {}),
                    "http_probe": {
                        "success": preflight_recon.get("http_probe", {}).get("success", False),
                        "status_code": preflight_recon.get("http_probe", {}).get("status_code"),
                        "title": preflight_recon.get("http_probe", {}).get("title", ""),
                        "content_type": preflight_recon.get("http_probe", {}).get("content_type", ""),
                    },
                },
            },
            temperature=0.1,
        ...
        if not runtime_available_tools:
            runtime_available_tools = [
                "nmap", "nuclei", "httpx", "katana", "ffuf", "dirsearch", "arjun", "paramspider",
                "dalfox", "amass", "subfinder", "gobuster", "feroxbuster", "wpscan", "sqlmap", "nikto"
            ]
        ...
        # Select optimal tools
        selected_tools = recon_planner.select_optimal_tools(profile, objective)
        selected_tools = [tool for tool in selected_tools if tool in runtime_available_tools]

        if isinstance(ai_strategy, dict):
            priority_tools = ai_strategy.get("tool_priority", [])
            if isinstance(priority_tools, list) and priority_tools:
                prioritized = [tool for tool in priority_tools if tool in runtime_available_tools]
                remaining = [tool for tool in selected_tools if tool not in prioritized]
                selected_tools = prioritized + remaining

        # Add fast-rotation tools early to maximize first findings.
        fast_rotation = [
            "httpx", "katana", "gau", "waybackurls", "arjun", "paramspider",
            "ffuf", "dirsearch", "dalfox", "nuclei", "nmap", "subfinder", "amass"
        ]
        for tool in fast_rotation:
            if tool in runtime_available_tools and tool not in selected_tools:
                selected_tools.append(tool)

        # Finally include the rest of runtime-available tools for broader coverage.
        for tool in runtime_available_tools:
            if tool not in selected_tools:
                selected_tools.append(tool)

        # Keep tool order stable while removing duplicates.
        selected_tools = list(dict.fromkeys(selected_tools))

        # Nikto is typically slower; keep it last or skip for speed-focused modes.
        if "nikto" in selected_tools:
            selected_tools = [tool for tool in selected_tools if tool != "nikto"] + ["nikto"]
            if objective in {"quick", "stealth"}:
                selected_tools = [tool for tool in selected_tools if tool != "nikto"]

        if not selected_tools:
            selected_tools = runtime_available_tools.copy()
            planning_warnings.append("Planner returned no executable tools; runtime executable pool applied")

        initial_batch_limit = min(8, max_tools)
        initial_tools = selected_tools[:initial_batch_limit]
        queued_followup_tools = [tool for tool in selected_tools if tool not in initial_tools]

        if not initial_tools:
            initial_tools = runtime_available_tools[:min(max_tools, 8)]
            planning_warnings.append("No tools selected by planner; default executable batch applied")

        ai_autonomous_recon: Dict[str, Any] = {
            "reasoning": "",
            "planned_steps": [],
            "executed_commands": [],
        }

        autonomous_step_templates: List[Dict[str, str]] = []
        if preflight_recon.get("web_detected"):
            autonomous_step_templates.append({
                "id": "curl_head_refresh",
                "name": "Refresh HTTP headers quickly",
            ...
            if shutil.which("katana"):
                autonomous_step_templates.append({
                    "id": "katana_quick_crawl",
                    "name": "Quick crawl",
                    "command": f"katana -u {safe_web_token} -silent -d 1 -jc -timeout 8",
                    "purpose": "Get first endpoint surface quickly",
                })
            if shutil.which("nuclei"):
                autonomous_step_templates.append({
                    "id": "nuclei_hot_scan",
                    "name": "Hot critical/high nuclei pass",
                    "command": f"nuclei -u {safe_web_token} -severity critical,high -silent -rl 50 -c 20",
                    "purpose": "Catch high-impact issues quickly",
                })
            if shutil.which("ffuf"):
                autonomous_step_templates.append({
                    "id": "ffuf_quick_dirs",
                    "name": "Quick directory fuzz",
                    "command": f"ffuf -u {safe_web_fuzz_token} -w /usr/share/wordlists/dirb/common.txt -mc all -fc 404 -t 40 -maxtime 20 -s",
                    "purpose": "Find interesting paths without waiting for long scans",
                })
        else:
            if shutil.which("ping"):
                ping_cmd = f"ping -n 1 {safe_host_token}" if os.name == "nt" else f"ping -c 1 {safe_host_token}"
                autonomous_step_templates.append({
                    "id": "ping_probe",
                    "name": "Host liveness check",
                    "command": ping_cmd,
                    "purpose": "Check if host responds quickly",
                })
            if shutil.which("nmap"):
                autonomous_step_templates.append({
                    "id": "nmap_top_ports",
                    "name": "Top ports discovery",
                    "command": f"nmap -Pn -T4 --top-ports 200 {safe_host_token}",
                    "purpose": "Fast network surface discovery",
                })
            if shutil.which("subfinder"):
                autonomous_step_templates.append({
                    "id": "subfinder_quick",
                    "name": "Quick subdomain lookup",
                    "command": f"subfinder -d {safe_host_token} -silent",
                    "purpose": "Find immediate subdomain targets",
                })

        max_autonomous_steps = 2 if objective == "quick" else 4
        if autonomous_step_templates:
            autonomous_plan_result = ollama_runtime.generate_json(
                prompt=(
                    "Return JSON with keys: reasoning (string) and selected_step_ids (list of strings). "
                    "Pick the fastest steps that maximize early findings."
                ),
                system_prompt=(
                    "You are DeepSec autonomous recon planner. Choose quick high-value steps first. "
                    "Prefer speed over deep exhaustive scans. Return strict JSON only."
                ),
                context={
                    "target": target,
                    "objective": objective,
                    ...
                    "preflight_recon": {
                        "target_kind": preflight_recon.get("target_kind"),
                        "target_hints": preflight_recon.get("target_hints", []),
                        "http_probe": preflight_recon.get("http_probe", {}),
                    },
                    "step_templates": [
                        {
                            "id": step["id"],
                        ...
                        for step in autonomous_step_templates
                    ],
                },
                temperature=0.1,
                num_predict=500,
            ...
            autonomous_plan = autonomous_plan_result.get("json", {}) if autonomous_plan_result.get("success") else {}
            ai_autonomous_recon["reasoning"] = str(autonomous_plan.get("reasoning", "")) if isinstance(autonomous_plan, dict) else ""

            if not autonomous_plan_result.get("success"):
                planning_warnings.append(
                    f"Autonomous quick-recon planning fallback used: {autonomous_plan_result.get('error', 'unknown error')}"
                )

            selected_step_ids = autonomous_plan.get("selected_step_ids", []) if isinstance(autonomous_plan, dict) else []
            template_by_id = {item["id"]: item for item in autonomous_step_templates}

            planned_steps = []
            if isinstance(selected_step_ids, list):
                planned_steps = [template_by_id[item_id] for item_id in selected_step_ids if item_id in template_by_id]

            if not planned_steps:
                planned_steps = autonomous_step_templates[:max_autonomous_steps]

            planned_steps = planned_steps[:max_autonomous_steps]
            ai_autonomous_recon["planned_steps"] = [step["id"] for step in planned_steps]
            ...
            for step in planned_steps:
                ...
                cmd_result = execute_command(step["command"], use_cache=False)
                stdout_text = str(cmd_result.get("stdout", "") or "")
                stderr_text = str(cmd_result.get("stderr", "") or "")
                cmd_success = bool(cmd_result.get("success", False))
                result_entry = {
                    "step_id": step["id"],
                    "name": step["name"],
                    "purpose": step["purpose"],
                    "command": step["command"],
                    "status": "success" if cmd_success else "failed",
                    "success": cmd_success,
                    "execution_time": cmd_result.get("execution_time", 0),
                    "stdout_excerpt": stdout_text[:700],
                    "stderr_excerpt": stderr_text[:300],
                    "error": str(cmd_result.get("error", "") or "")[:220],
                }
                ai_autonomous_recon["executed_commands"].append(result_entry)
                ...
                live_event_bus.emit(
                    ...
                    data={
                        "step": step["id"],
                        "stdout_excerpt": stdout_text[:220],
                        "stderr_excerpt": stderr_text[:220],
                    },
                )
        ...
        live_event_bus.emit(
            ...
            trace_id=trace_id,
            data={
                "phase_1_tools": initial_tools,
                "queued_tools": queued_followup_tools,
                "tool_budget": max_tools,
                "auto_budget": auto_budget,
            },
        )
        ...
        scan_results = {
            ...
            "autonomous_decision": autonomous_decision,
            "preflight_recon": preflight_recon,
            "ai_autonomous_recon": ai_autonomous_recon,
            "target_profile": profile.to_dict(),
            "tools_executed": [],
        ...
        def execute_tool_via_api_endpoint(tool_name: str, effective_target: str, optimized_params: Dict[str, Any]) -> Dict[str, Any]:
            ...
            payload.setdefault("domain", host_no_port)
            payload.setdefault("host", host_no_port)
            payload.setdefault("use_recovery", True)

            if port_hint and not payload.get("ports") and tool_name in {"nmap", "nmap-advanced", "masscan", "rustscan"}:
                payload["ports"] = str(port_hint)

            # Lightweight defaults to keep scans responsive.
            if objective in {"quick", "stealth"}:
                payload.setdefault("threads", 35)
                payload.setdefault("timeout", 20)
            else:
                payload.setdefault("threads", 20)
                payload.setdefault("timeout", 35)

            # Target-shape specialization for common endpoint contracts.
            web_route_tools = {
                "gobuster", "nuclei", "nikto", "sqlmap", "ffuf", "feroxbuster", "katana",
                "wpscan", "dirsearch", "arjun", "dalfox", "jaeles", "xsser", "wfuzz", "x8",
                "http-framework", "browser-agent", "burpsuite-alternative", "zap"
            }
            domain_route_tools = {"amass", "subfinder", "paramspider", "fierce", "dnsenum", "gau", "waybackurls"}
            network_route_tools = {
                "nmap", "nmap-advanced", "masscan", "rustscan", "enum4linux", "enum4linux-ng",
                "rpcclient", "smbmap", "netexec", "nbtscan", "arp-scan", "responder"
            }

            if tool_name in web_route_tools:
                payload["url"] = web_target
                payload["target"] = web_target
            elif tool_name in domain_route_tools:
                payload["domain"] = host_no_port
                payload["target"] = host_no_port
            elif tool_name in network_route_tools:
                payload["target"] = host_no_port

            if tool_name == "httpx":
                payload["target"] = web_target
                payload.setdefault("tech_detect", True)
                payload.setdefault("status_code", True)
                payload.setdefault("title", True)

            if tool_name == "nuclei":
                payload.setdefault("severity", "critical,high" if objective in {"quick", "stealth"} else "critical,high,medium")

            if "additional_args" not in payload:
                payload["additional_args"] = ""
            ...
            try:
                ...
                if response.status_code == 404:
                    return {
                        "success": False,
                        "error": f"Tool endpoint not found: {endpoint_path}",
                        "stdout": "",
                        "stderr": "",
                        "command": "",
                        "execution_time": 0,
                ...
                if response.status_code >= 400:
                    error_msg = response_data.get("error") or response_data.get("details") or f"HTTP {response.status_code}"
                    return {
                        "success": False,
                        "error": str(error_msg),
                        "stdout": str(response_data.get("stdout", "") or ""),
                        "stderr": str(response_data.get("stderr", "") or ""),
                        "command": str(response_data.get("command", "") or ""),
                        "execution_time": float(response_data.get("execution_time", 0) or 0),
                    }

                success_value = response_data.get("success")
                if success_value is None:
                    success_value = not bool(response_data.get("error"))

                stdout_value = response_data.get("stdout", "")
                if not stdout_value and response_data.get("output"):
                    stdout_value = response_data.get("output")
                if not stdout_value and isinstance(response_data.get("results"), (dict, list)):
                    stdout_value = json.dumps(response_data.get("results"), default=str)
                ...
                return {
                    "success": False,
                    "error": f"Endpoint execution failed for {tool_name}: {str(endpoint_exc)}",
                    "stdout": "",
                    "stderr": "",
                    "command": "",
                    "execution_time": 0,
        ...
        def execute_single_tool(tool_name, target, profile):
            ...
            try:
                ...
                network_tools = {'nmap', 'nmap-advanced', 'masscan', 'rustscan'}

                if tool_name in web_tools:
                    effective_target = web_target
                elif tool_name in domain_tools:
                    effective_target = host_no_port
                else:
                    effective_target = host_no_port
                ...
                if tool_name in tool_execution_map:
                    ...
                    result = execute_tool_via_api_endpoint(tool_name, effective_target, optimized_params)
        ...
        adaptive_phase_notes: List[Dict[str, Any]] = []
        executed_tool_names: Set[str] = set()

        def execute_tool_batch(tool_batch: List[str], phase_label: str) -> None:
            """Run one phase of tools in parallel and aggregate results into scan_results."""
            if not tool_batch:
                return

            live_event_bus.emit(
                category="scan-status",
                message=f"Starting {phase_label} with {len(tool_batch)} tools",
                trace_id=trace_id,
                data={"phase": phase_label, "tools": tool_batch},
            )

            with ThreadPoolExecutor(max_workers=min(len(tool_batch), 5)) as executor:
                future_to_tool = {
                ...
                for future in as_completed(future_to_tool):
                    ...
                    completed_count = len(scan_results["tools_executed"])
                    live_event_bus.emit(
                        category="scan-status",
                        message=(
                        ...
                        trace_id=trace_id,
                        data={"completed": completed_count, "total": max_tools, "phase": phase_label},
                    )
        ...
        phase1_tools = [tool for tool in initial_tools if tool not in executed_tool_names][:max_tools]
        execute_tool_batch(phase1_tools, "phase-1")
        ...
        phase_index = 2
        while len(scan_results["tools_executed"]) < max_tools and phase_index <= 5:
            remaining_budget = max_tools - len(scan_results["tools_executed"])
            ...
            if queued_followup_tools:
                phase_tools = queued_followup_tools[:min(remaining_budget, 5)]
                queued_followup_tools = queued_followup_tools[len(phase_tools):]
                phase_reason = "Continuing queued planner tools"
            else:
                followup_candidates = [tool for tool in runtime_available_tools if tool not in executed_tool_names]
                if not followup_candidates:
                    break

                followup_plan_result = ollama_runtime.generate_json(
                    prompt=(
                        "Return JSON with keys: next_tools (list of strings), reasoning (string), and stop (boolean). "
                        "Choose only from followup_candidates."
                    ),
                    system_prompt=(
                        "You are DeepSec adaptive scan orchestrator for explicitly authorized testing. "
                        "If enough evidence is collected, set stop=true. Otherwise return the next best tools. "
                        "Return strict JSON only."
                    ),
                    context={
                        "target": target,
                        ...
                        "execution_summary": {
                            "executed_tools": [item.get("tool") for item in scan_results["tools_executed"]],
                            "total_findings": scan_results["total_vulnerabilities"],
                            "successful_tools": len([t for t in scan_results["tools_executed"] if t.get("success")]),
                            "failed_tools": len([t for t in scan_results["tools_executed"] if not t.get("success")]),
                        },
                        "recent_tool_digests": [
                            {
                                "tool": item.get("tool"),
                            ...
                            for item in scan_results["tools_executed"][-8:]
                        ],
                    },
                    temperature=0.1,
                    num_predict=500,
                ...
                followup_plan = followup_plan_result.get("json", {}) if followup_plan_result.get("success") else {}

                if not followup_plan_result.get("success"):
                    warning_text = f"Adaptive follow-up planning failed; using heuristic continuation: {followup_plan_result.get('error', 'unknown error')}"
                ...
                elif followup_plan_result.get("parse_error"):
                    warning_text = followup_plan_result.get("warning", "Adaptive follow-up parse fallback applied")
                    planning_warnings.append(warning_text)
                    live_event_bus.emit(
                        category="ai-thought",
                ...
                requested_next_tools = followup_plan.get("next_tools", []) if isinstance(followup_plan, dict) else []
                if isinstance(requested_next_tools, list):
                    phase_tools = [
                        str(tool).strip()
                        for tool in requested_next_tools
                        if str(tool).strip() in followup_candidates
                    ]
                ...
                if not phase_tools:
                    heuristic_priority = [
                        "httpx", "katana", "arjun", "paramspider", "nuclei", "sqlmap",
                        "ffuf", "dirsearch", "dalfox", "nikto", "nmap", "subfinder",
            ...
            execute_tool_batch(phase_tools, f"phase-{phase_index}")
            executed_tool_names.update(phase_tools)
            adaptive_phase_notes.append({
        ...
        total_executed_tools = len(scan_results["tools_executed"])

        scan_results["execution_summary"] = {
            "total_tools": total_executed_tools,
            "successful_tools": len(successful_tools),
            "failed_tools": len(failed_tools),
            "success_rate": len(successful_tools) / total_executed_tools * 100 if total_executed_tools else 0,
            ...
            "tool_budget": max_tools,
            "adaptive_phases": adaptive_phase_notes,
        }

        # Command replay and concise digests for post-scan reporting.
        autonomous_command_replay = [
            {
                "tool": f"ai-quick::{item.get('step_id', 'step')}",
                "status": item.get("status", "unknown"),
                "effective_target": web_target if preflight_recon.get("web_detected") else host_target,
                "command": item.get("command", ""),
                "error": item.get("error", "") or item.get("stderr_excerpt", ""),
                "vulnerabilities_found": 0,
            }
            for item in ai_autonomous_recon.get("executed_commands", [])
        ]

        command_replay = autonomous_command_replay + [
            {
                "tool": item.get("tool"),
                "status": item.get("status"),
        ...
        ai_summary_result = ollama_runtime.generate_json(
            ...
            context={
                ...
                "autonomous_quick_checks": ai_autonomous_recon,
        ...
        report_lines.append("DEEPSEC OPERATION REPORT")
        report_lines.append("")
        report_lines.append(f"Target: {target}")
        report_lines.append(f"Mode: {'AUTONOMOUS' if autonomous_mode else 'OPERATOR-ASSISTED'}")
        ...
        if preflight_recon.get("http_probe", {}).get("success"):
            report_lines.append(
                f"- HTTP: {preflight_recon.get('http_probe', {}).get('status_code', 'n/a')} | "
                f"{preflight_recon.get('http_probe', {}).get('content_type', 'n/a')} | "
                f"title={preflight_recon.get('http_probe', {}).get('title', '')}"
            )
        else:
        ...
        for idx, item in enumerate(ai_autonomous_recon.get("executed_commands", []), start=1):
            ...
            excerpt = (item.get("stdout_excerpt", "") or item.get("stderr_excerpt", "") or "").strip()
            if excerpt:
                report_lines.append(f"  excerpt: {excerpt[:220]}")
            if item.get("error"):
                report_lines.append(f"  error: {item.get('error')}")
        if not ai_autonomous_recon.get("executed_commands"):
            report_lines.append("- No autonomous quick command executed")

        report_lines.append("")
        report_lines.append("3) ADAPTIVE TOOL EXECUTION")
        report_lines.append(
            f"- Tools success: {len(successful_tools)}/{total_executed_tools} | "
            f"Potential findings: {scan_results['total_vulnerabilities']}"
        )
        for phase in scan_results.get("execution_summary", {}).get("adaptive_phases", []):
            report_lines.append(
                f"- {phase.get('phase', 'phase')}: {', '.join(phase.get('tools', []))} | reason: {phase.get('reason', '')}"
            )
        ...
        for step in ai_scan_summary.get("recommended_next_steps", [])[:10]:
            report_lines.append(f"- {step}")
        if not ai_scan_summary.get("recommended_next_steps"):
            report_lines.append("- Re-run with broader objective if additional coverage is required")

        report_lines.append("")
        report_lines.append("6) COMMAND TIMELINE")
        for idx, item in enumerate(command_replay, start=1):
            report_lines.append(
                f"- [{idx}] {item.get('tool', 'tool')} -> {item.get('status', 'unknown')} | {item.get('command', '')}"
            )
            if item.get("error"):
                report_lines.append(f"  error: {item.get('error')}")
        ...
        logger.info(f" Results: {len(successful_tools)}/{total_executed_tools} tools successful, {scan_results['total_vulnerabilities']} vulnerabilities found")
...
def detect_technologies():
    ...
    try:
        ...
        ai_result = ollama_runtime.generate_json(
            prompt=(
                "Return JSON with keys: technology_recommendations (object), "
                "priority_testing_order (list of strings), and notes (list of strings)."
            ),
            system_prompt=(
                "You are DeepSec AI technology analyst. Return strict JSON only."
            ),
            context={
                "target": target,
                "detected_technologies": [tech.value for tech in profile.technologies],
                "cms_type": profile.cms_type,
                "baseline_recommendations": tech_recommendations,
            },
            temperature=0.1,
            num_predict=700,
        )

        ai_runtime = build_ai_runtime_metadata("technology-detection", ai_result)
        if not ai_result.get("success") and AI_ENFORCE_OLLAMA:
            return jsonify({
                "success": False,
                "error": "Ollama runtime failure during technology detection",
                "details": ai_result.get("error", "Unknown error"),
                "ai_runtime": ai_runtime,
                "timestamp": datetime.now().isoformat(),
            }), 503

        ai_recommendations = ai_result.get("json", {}) if ai_result.get("success") else {}
        ai_tech_map = ai_recommendations.get("technology_recommendations", {}) if isinstance(ai_recommendations, dict) else {}
        if isinstance(ai_tech_map, dict):
            tech_recommendations.update(ai_tech_map)
...
def pacu():
    ...
    try:
        params = request.json
        session_name = params.get("session_name", "deepsec_session")
...
def ghidra():
    ...
    try:
        params = request.json
        binary = params.get("binary", "")
        project_name = params.get("project_name", "deepsec_analysis")
        script_file = params.get("script_file", "")
...
class HTTPTestingFramework:
    ...
    def __init__(self):
        ...
        self.session.headers.update({
            'User-Agent': 'DeepSec-HTTP-Framework/1.0 (Advanced Security Testing)'
        })
        self.proxy_history = []
    ...
    def intercept_request(self, url: str, method: str = 'GET', data: dict = None,
        ...
        try:
            ...
            logger.error(f"{NovaRenderCore.format_error_card('ERROR', 'HTTP-Framework', str(e))}")
            return {'success': False, 'error': str(e)}
    ...
    def _apply_match_replace(self, url: str, data, headers: dict):
        ...
        if not self._in_scope(url):
            logger.warning(f"{NovaRenderCore.format_tool_status('HTTP-Framework', 'SKIPPED', f'Out of scope: {url}')}" )
    ...
    def spider_website(self, base_url: str, max_depth: int = 3, max_pages: int = 100) -> dict:
        ...
        try:
            ...
            logger.error(f"{NovaRenderCore.format_error_card('ERROR', 'Spider', str(e))}")
            return {'success': False, 'error': str(e)}
...
class BrowserAgent:
    ...
    def setup_browser(self, headless: bool = True, proxy_port: int = None):
        ...
        try:
            ...
            chrome_options.add_argument('--user-agent=DeepSec-BrowserAgent/1.0 (Security Testing)')
            ...
            logger.info(f"{NovaRenderCore.format_tool_status('BrowserAgent', 'RUNNING', 'Chrome Browser Initialized')}")
            return True
    ...
    def navigate_and_inspect(self, url: str, wait_time: int = 5) -> dict:
        ...
        try:
            ...
            logger.info(f"{NovaRenderCore.format_command_execution(nav_command, 'STARTING')}")
            ...
            # Take screenshot
            screenshot_path = f"/tmp/deepsec_screenshot_{int(time.time())}.png"
            ...
            logger.info(f"{NovaRenderCore.format_tool_status('BrowserAgent', 'SUCCESS', url)}")
    ...
    def run_active_tests(self, page_info: dict, payload: str = '<deepsecXSSTest123>') -> dict:
    ...
    def close_browser(self):
        ...
        if self.driver:
            self.driver.quit()
            self.driver = None
            logger.info(f"{NovaRenderCore.format_tool_status('BrowserAgent', 'SUCCESS', 'Browser Closed')}")
...
def http_framework_endpoint():
    ...
    try:
        ...
        logger.info(f"{NovaRenderCore.create_section_header('HTTP FRAMEWORK', '', 'FIRE_RED')}")
        ...
        if action == "request":
            ...
            logger.info(f"{NovaRenderCore.format_command_execution(request_command, 'STARTING')}")
            result = http_framework.intercept_request(url, method, data, headers, cookies)

            if result.get("success"):
                logger.info(f"{NovaRenderCore.format_tool_status('HTTP-Framework', 'SUCCESS', url)}")
            else:
                logger.error(f"{NovaRenderCore.format_tool_status('HTTP-Framework', 'FAILED', url)}")
            ...
            logger.info(f"{NovaRenderCore.format_command_execution(spider_command, 'STARTING')}")
            result = http_framework.spider_website(url, max_depth, max_pages)

            if result.get("success"):
                total_pages = result.get("total_pages", 0)
                pages_info = f"{total_pages} pages"
                logger.info(f"{NovaRenderCore.format_tool_status('HTTP-Spider', 'SUCCESS', pages_info)}")
            else:
                logger.error(f"{NovaRenderCore.format_tool_status('HTTP-Spider', 'FAILED', url)}")
...
def browser_agent_endpoint():
    ...
    try:
        ...
        logger.info(
            f"{NovaRenderCore.create_section_header('BROWSER AGENT', '', 'CRIMSON')}"
        )
        ...
        if action == "navigate":
            ...
            if result.get("success") and active_tests:
                ...
                if active_results["active_findings"]:
                    logger.warning(
                        NovaRenderCore.format_error_card(
                            "WARNING",
                            "BrowserAgent",
        ...
        logger.error(
            f"{NovaRenderCore.format_error_card('ERROR', 'BrowserAgent', str(e))}"
        )
        return jsonify({"error": f"Server error: {str(e)}"}), 500
...
def burpsuite_alternative():
    ...
    try:
        ...
        logger.info(f"{NovaRenderCore.create_section_header('BURP SUITE ALTERNATIVE', '', 'BLOOD_RED')}")
        scan_message = f'Starting {scan_type} scan of {target}'
        logger.info(f"{NovaRenderCore.format_highlighted_text(scan_message, 'RED')}")
        ...
        if scan_type in ['comprehensive', 'spider']:
            logger.info(f"{NovaRenderCore.format_tool_status('BrowserAgent', 'RUNNING', 'Reconnaissance Phase')}")
        ...
        if scan_type in ['comprehensive', 'spider']:
            logger.info(f"{NovaRenderCore.format_tool_status('HTTP-Spider', 'RUNNING', 'Discovery Phase')}")
        ...
        if scan_type in ['comprehensive', 'active']:
            logger.info(f"{NovaRenderCore.format_tool_status('VulnScanner', 'RUNNING', 'Analysis Phase')}")
        ...
        logger.info(f"{NovaRenderCore.create_section_header('SCAN COMPLETE', '', 'SUCCESS')}")
        vuln_message = f'Found {total_vulns} vulnerabilities'
        color_choice = 'YELLOW' if total_vulns > 0 else 'GREEN'
        logger.info(f"{NovaRenderCore.format_highlighted_text(vuln_message, color_choice)}")

        for severity, count in vuln_summary.items():
            logger.info(f"  {NovaRenderCore.format_vulnerability_severity(severity, count)}")

        return jsonify(results)

    except Exception as e:
        logger.error(f"{NovaRenderCore.format_error_card('CRITICAL', 'BurpAlternative', str(e))}")
        return jsonify({"error": f"Server error: {str(e)}"}), 500
...
def ai_generate_payload():
    ...
    try:
        ...
        ai_result = ollama_runtime.generate_json(
            prompt=(
                "Return JSON with keys: payloads (list of objects), recommendations (list of strings), "
                "and model_notes (string). Each payload object must contain payload, context, encoding, risk_level."
            ),
            system_prompt=(
                "You are DeepSec AI payload engineer using authorized testing context only. Return strict JSON only."
            ),
            context={
                "target_info": target_info,
                "baseline_payload_generation": result,
            },
            temperature=0.15,
            num_predict=1000,
        )

        ai_runtime = build_ai_runtime_metadata("ai-generate-payload", ai_result)
        if not ai_result.get("success") and AI_ENFORCE_OLLAMA:
            return jsonify({
                "success": False,
                "error": "Ollama runtime failure during payload generation",
                "details": ai_result.get("error", "Unknown error"),
                "ai_runtime": ai_runtime,
                "timestamp": datetime.now().isoformat(),
            }), 503

        ai_payload = ai_result.get("json", {}) if ai_result.get("success") else {}
        ai_payloads = ai_payload.get("payloads", []) if isinstance(ai_payload, dict) else []
        if isinstance(ai_payloads, list) and ai_payloads:
            sanitized_payloads = []
            for item in ai_payloads[:40]:
                ...
                sanitized_payloads.append({
                    ...
                    "context": str(item.get("context", "ai_generated")),
                    "encoding": str(item.get("encoding", "none")),
                    "risk_level": str(item.get("risk_level", "MEDIUM")).upper(),
                })

            if sanitized_payloads:
                result["payloads"] = sanitized_payloads
                result["payload_count"] = len(sanitized_payloads)

        ai_recommendations = ai_payload.get("recommendations", []) if isinstance(ai_payload, dict) else []
        if isinstance(ai_recommendations, list) and ai_recommendations:
            result["recommendations"] = [str(item) for item in ai_recommendations[:20]]

        result["ai_model_notes"] = ai_payload.get("model_notes", "") if isinstance(ai_payload, dict) else ""
...
def ai_test_payload():
    ...
    try:
        ...
        ai_result = ollama_runtime.generate_json(
            prompt=(
                "Return JSON with keys: potential_vulnerability (boolean), severity (string), "
                "analysis_summary (string), and recommendations (list of strings)."
            ),
            system_prompt=(
                "You are DeepSec AI payload test reviewer. Return strict JSON only based on test evidence."
            ),
            context={
                "payload": payload,
                "target_url": target_url,
        ...
        ai_runtime = build_ai_runtime_metadata("ai-test-payload", ai_result)
        if not ai_result.get("success") and AI_ENFORCE_OLLAMA:
            return jsonify({
                "success": False,
                "error": "Ollama runtime failure during payload testing",
                "details": ai_result.get("error", "Unknown error"),
                "ai_runtime": ai_runtime,
                "timestamp": datetime.now().isoformat(),
            }), 503

        if ai_result.get("success") and isinstance(ai_result.get("json"), dict):
            model_analysis = ai_result["json"]
            if "potential_vulnerability" in model_analysis:
                analysis["potential_vulnerability"] = bool(model_analysis["potential_vulnerability"])
            if "severity" in model_analysis:
                analysis["severity"] = str(model_analysis["severity"])
            if "analysis_summary" in model_analysis:
                analysis["analysis_summary"] = str(model_analysis["analysis_summary"])
            if isinstance(model_analysis.get("recommendations"), list):
                analysis["recommendations"] = [str(item) for item in model_analysis["recommendations"][:20]]

            analysis["model_review"] = model_analysis
        else:
...
def exploit_generate():
    ...
    try:
        ...
        ai_result = ollama_runtime.generate_json(
            prompt=(
                "Return JSON with keys: exploit_review (string), deployment_notes (list of strings), "
                "and risk_controls (list of strings)."
            ),
            system_prompt=(
                "You are DeepSec AI exploit reviewer. Return strict JSON only with authorized testing guidance."
            ),
            context={
                "cve_data": cve_data,
                "target_info": target_info,
        ...
        ai_runtime = build_ai_runtime_metadata("exploit-generate", ai_result)
        if not ai_result.get("success") and AI_ENFORCE_OLLAMA:
            return jsonify({
                "success": False,
                "error": "Ollama runtime failure during exploit generation",
                "details": ai_result.get("error", "Unknown error"),
                "ai_runtime": ai_runtime,
                "timestamp": datetime.now().isoformat(),
            }), 503

        ai_enhancement = ai_result.get("json", {}) if ai_result.get("success") else {}
...
def advanced_payload_generation():
    ...
    try:
        ...
        ai_result = ollama_runtime.generate_json(
            prompt=(
                "Return JSON with keys: advanced_payloads (list), deployment_guide (object), "
                "operator_notes (list of strings)."
            ),
            system_prompt=(
                "You are DeepSec AI payload strategist. Return strict JSON only for authorized testing contexts."
            ),
            context={
                "attack_type": attack_type,
                ...
                "baseline_payloads": advanced_payloads,
                "baseline_guide": deployment_guide,
            },
            temperature=0.15,
            num_predict=1000,
        )

        ai_runtime = build_ai_runtime_metadata("advanced-payload-generation", ai_result)
        if not ai_result.get("success") and AI_ENFORCE_OLLAMA:
            return jsonify({
                "success": False,
                "error": "Ollama runtime failure during advanced payload generation",
                "details": ai_result.get("error", "Unknown error"),
                "ai_runtime": ai_runtime,
                "timestamp": datetime.now().isoformat(),
            }), 503

        ai_overlay = ai_result.get("json", {}) if ai_result.get("success") else {}
        ai_payloads = ai_overlay.get("advanced_payloads", []) if isinstance(ai_overlay, dict) else []
        if isinstance(ai_payloads, list) and ai_payloads:
            advanced_payloads = ai_payloads[:25]

        ai_deployment_guide = ai_overlay.get("deployment_guide", {}) if isinstance(ai_overlay, dict) else {}
        if isinstance(ai_deployment_guide, dict) and ai_deployment_guide:
            deployment_guide = ai_deployment_guide
        ...
        result = {
            ...
            "advanced_payload_generation": {
                ...
                "operator_notes": ai_overlay.get("operator_notes", []) if isinstance(ai_overlay, dict) else []
            },
...
def get_error_statistics():
    ...
    try:
        stats = resilience_engine.get_error_statistics()
        return jsonify({
            "success": True,
            "statistics": stats,
            "timestamp": datetime.now().isoformat()
...
def test_error_recovery():
    ...
    try:
        ...
        return jsonify({
            ...
            "error_classification": resilience_engine.classify_error(str(exception), exception).value,
            "alternative_tools": resilience_engine.tool_alternatives.get(tool_name, []),
            "timestamp": datetime.now().isoformat()
        })
...
def get_fallback_chains():
    ...
    try:
        ...
        if operation:
            fallback_chain = continuity_manager.create_fallback_chain(operation, failed_tools)
            return jsonify({
                "success": True,
                "operation": operation,
                "fallback_chain": fallback_chain,
                "is_critical": continuity_manager.is_critical_operation(operation),
                "timestamp": datetime.now().isoformat()
            })
        else:
            return jsonify({
                "success": True,
                "available_operations": list(continuity_manager.fallback_chains.keys()),
                "critical_operations": list(continuity_manager.critical_operations),
                "timestamp": datetime.now().isoformat()
            })
...
def classify_error_endpoint():
    ...
    try:
        ...
        error_type = resilience_engine.classify_error(error_message)
        recovery_strategies = resilience_engine.recovery_strategies.get(error_type, [])
...
def get_parameter_adjustments():
    ...
    try:
        ...
        adjusted_params = resilience_engine.auto_adjust_parameters(tool_name, error_type, original_params)
...
def get_alternative_tools():
    ...
    try:
        ...
        alternatives = resilience_engine.tool_alternatives.get(tool_name, [])
...
OpsRenderCore = NovaRenderCore
BANNER = NovaRenderCore.create_banner()

if __name__ == "__main__":
    # Display the startup banner
    print(BANNER)

    parser = argparse.ArgumentParser(description="Run the DeepSec Control Plane API")
    parser.add_argument("--debug", action="store_true", help="Enable debug mode")
    parser.add_argument("--port", type=int, default=API_PORT, help=f"Port for the API server (default: {API_PORT})")
    parser.add_argument("--production", action="store_true", help="Run with Waitress production server")
    parser.add_argument("--threads", type=int, default=8, help="Worker threads for production mode")
    ...
    # Startup messages with structured formatting
    startup_info = f"""
{NovaRenderCore.COLORS['MATRIX_GREEN']}{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['NEON_BLUE']} Starting DeepSec Control Plane API{NovaRenderCore.COLORS['RESET']}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['CYBER_ORANGE']} Port:{NovaRenderCore.COLORS['RESET']} {API_PORT}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['WARNING']} Debug Mode:{NovaRenderCore.COLORS['RESET']} {DEBUG_MODE}
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['ELECTRIC_PURPLE']} Cache Size:{NovaRenderCore.COLORS['RESET']} {CACHE_SIZE} | TTL: {CACHE_TTL}s
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['TERMINAL_GRAY']}  Command Timeout:{NovaRenderCore.COLORS['RESET']} {COMMAND_TIMEOUT}s
{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']} {NovaRenderCore.COLORS['MATRIX_GREEN']} Terminal Presentation Layer:{NovaRenderCore.COLORS['RESET']} Active
{NovaRenderCore.COLORS['MATRIX_GREEN']}{NovaRenderCore.COLORS['BOLD']}{NovaRenderCore.COLORS['RESET']}
"""
    ...
    ai_boot_health = ollama_runtime.health()
    if ai_boot_health.get("success"):
        logger.info(
            f" AI runtime online | Model: {OLLAMA_MODEL} | Ready: {ai_boot_health.get('model_ready', False)}"
        )
    else:
        logger.error(f" AI runtime offline: {ai_boot_health.get('error', 'Unknown error')}")
        if AI_ENFORCE_OLLAMA:
            logger.error(" AI enforcement is enabled. AI endpoints will fail until Ollama is reachable.")
    ...
    if args.production:
        try:
            ...
            serve = waitress_module.serve

            logger.info(f" Starting Waitress production server | Threads: {args.threads}")
            serve(app, host="0.0.0.0", port=API_PORT, threads=max(2, args.threads))
        except Exception as exc:
            logger.error(f" Failed to start Waitress server: {exc}")
