#!/usr/bin/env python3
"""
DeepSec Bridge Client - Autonomous Agent Orchestration Interface
Author: TheDeepOpc
...
CURRENT CHARACTERISTICS:
 ...
 Structured terminal output with consistent styling
 ...
 Advanced structured logging with clean textual severity labels

Architecture: MCP Client for AI agent communication with DeepSec server
...
# Compatibility aliases preserve existing integrations.
requests = netreq

class OpsPalette:
    """Operational color palette aligned with server terminal styling."""
    ...
    # Core enhanced colors (neon-cyan profile)
    MATRIX_GREEN = '\033[38;5;50m'
    NEON_BLUE = '\033[38;5;45m'
    ELECTRIC_PURPLE = '\033[38;5;141m'
    CYBER_ORANGE = '\033[38;5;214m'
    HACKER_RED = '\033[38;5;45m'
    ...
    # Compatibility aliases retained and remapped to the current palette
    BLOOD_RED = '\033[38;5;39m'
    CRIMSON = '\033[38;5;44m'
    DARK_RED = '\033[38;5;31m'
    FIRE_RED = '\033[38;5;81m'
    ROSE_RED = '\033[38;5;75m'
    BURGUNDY = '\033[38;5;24m'
    SCARLET = '\033[38;5;117m'
    RUBY = '\033[38;5;111m'

    # Highlighting colors
    HIGHLIGHT_RED = '\033[48;5;39m\033[38;5;15m'  # Cyan background, white text
    HIGHLIGHT_YELLOW = '\033[48;5;226m\033[38;5;16m'  # Yellow background, black text
    HIGHLIGHT_GREEN = '\033[48;5;50m\033[38;5;16m'  # Teal background, black text
    HIGHLIGHT_BLUE = '\033[48;5;45m\033[38;5;16m'  # Cyan-blue background, black text
    HIGHLIGHT_PURPLE = '\033[48;5;141m\033[38;5;15m'  # Violet background, white text

    # Status colors with cool neon tones
    SUCCESS = '\033[38;5;50m'  # Neon teal
    WARNING = '\033[38;5;214m'  # Amber
    ERROR = '\033[38;5;203m'  # Coral
    CRITICAL = '\033[48;5;203m\033[38;5;15m\033[1m'  # Coral background, white bold text
    INFO = '\033[38;5;45m'  # Cyan-blue
    DEBUG = '\033[38;5;240m'  # Gray

    # Vulnerability severity colors
    VULN_CRITICAL = '\033[48;5;203m\033[38;5;15m\033[1m'  # Coral background
    VULN_HIGH = '\033[38;5;203m\033[1m'  # Coral bold
    VULN_MEDIUM = '\033[38;5;214m\033[1m'  # Amber bold
    VULN_LOW = '\033[38;5;117m'  # Soft cyan
    VULN_INFO = '\033[38;5;45m'  # Cyan-blue

    # Tool status colors
    TOOL_RUNNING = '\033[38;5;50m\033[5m'  # Blinking teal
    TOOL_SUCCESS = '\033[38;5;50m\033[1m'  # Bold teal
    TOOL_FAILED = '\033[38;5;203m\033[1m'  # Bold coral
    TOOL_TIMEOUT = '\033[38;5;214m\033[1m'  # Bold amber
    TOOL_RECOVERY = '\033[38;5;141m\033[1m'  # Bold violet

# Backward compatibility aliases
AegisPalette = OpsPalette
Colors = AegisPalette

class ColoredFormatter(logging.Formatter):
    """Structured color formatter for MCP client logs."""

    COLORS = {
        'DEBUG': AegisPalette.DEBUG,
        'INFO': AegisPalette.SUCCESS,
        'WARNING': AegisPalette.WARNING,
        'ERROR': AegisPalette.ERROR,
        'CRITICAL': AegisPalette.CRITICAL
    }
    ...
    def format(self, record):
        ...
        color = self.COLORS.get(record.levelname, AegisPalette.BRIGHT_WHITE)

        # Add color and emoji to the message
        record.msg = f"{color}{emoji} {record.msg}{AegisPalette.RESET}"
        return super().format(record)

# Setup logging
logging.basicConfig(
    level=logging.INFO,
    format="[ DeepSec MCP] %(asctime)s [%(levelname)s] %(message)s",
    handlers=[
...
for handler in logging.getLogger().handlers:
    handler.setFormatter(ColoredFormatter(
        "[ DeepSec MCP] %(asctime)s [%(levelname)s] %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S"
    ))
...
DEFAULT_CONTROL_PLANE_URL = "http://127.0.0.1:8888"  # Default DeepSec server URL
DEFAULT_REQUEST_TIMEOUT = 300  # 5 minutes default timeout for API requests
MAX_RETRIES = 3  # Maximum number of retries for connection attempts

class RelayOpsClient:
    """Structured client for communicating with the DeepSec Control Plane API"""

    def __init__(self, server_url: str, timeout: int = DEFAULT_REQUEST_TIMEOUT):
        """
        Initialize the DeepSec Control Plane client

        Args:
            server_url: URL of the DeepSec Control Plane API
        ...
        for i in range(MAX_RETRIES):
            try:
                logger.info(f" Attempting to connect to DeepSec Control Plane API at {server_url} (attempt {i+1}/{MAX_RETRIES})")
                ...
                try:
                    ...
                    connected = True
                    logger.info(f" Successfully connected to DeepSec Control Plane API at {server_url}")
                    ...
                    logger.warning(f" Connection refused to {server_url}. Make sure the DeepSec Control Plane server is running.")
        ...
        if not connected:
            error_msg = f"Failed to establish connection to DeepSec Control Plane API at {server_url} after {MAX_RETRIES} attempts"
    ...
    def execute_command(self, command: str, use_cache: bool = True) -> Dict[str, Any]:
        """
        Execute a generic command on the DeepSec server
    ...
    def check_health(self) -> Dict[str, Any]:
        """
        Check the health of the DeepSec Control Plane API
...
def assemble_tool_mesh(relay_client: RelayOpsClient) -> FastMCP:
    """
    Set up the MCP server with all enhanced tool functions

    Args:
        relay_client: Initialized RelayOpsClient

    Returns:
        Configured FastMCP instance
    """
    mcp = FastMCP("deepsec-core-mcp")
    ...
    def nmap_scan(target: str, scan_type: str = "-sV", ports: str = "", additional_args: str = "") -> Dict[str, Any]:
        ...
        logger.info(f"{AegisPalette.FIRE_RED} Initiating Nmap scan: {target}{AegisPalette.RESET}")

        # Use enhanced error handling by default
        data["use_recovery"] = True
        result = relay_client.safe_post("api/tools/nmap", data)

        if result.get("success"):
            logger.info(f"{AegisPalette.SUCCESS} Nmap scan completed successfully for {target}{AegisPalette.RESET}")
            ...
            if result.get("recovery_info", {}).get("recovery_applied"):
                ...
                logger.info(f"{AegisPalette.HIGHLIGHT_YELLOW} Recovery applied: {attempts} attempts made {AegisPalette.RESET}")
        else:
            logger.error(f"{AegisPalette.ERROR} Nmap scan failed for {target}{AegisPalette.RESET}")

            # Check for human escalation
            if result.get("human_escalation"):
                logger.error(f"{AegisPalette.CRITICAL} HUMAN ESCALATION REQUIRED {AegisPalette.RESET}")
    ...
    def gobuster_scan(url: str, mode: str = "dir", wordlist: str = "/usr/share/wordlists/dirb/common.txt", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Gobuster to find directories, DNS subdomains, or virtual hosts with structured logging.
        ...
        logger.info(f"{AegisPalette.CRIMSON} Starting Gobuster {mode} scan: {url}{AegisPalette.RESET}")

        # Use enhanced error handling by default
        data["use_recovery"] = True
        result = relay_client.safe_post("api/tools/gobuster", data)

        if result.get("success"):
            logger.info(f"{AegisPalette.SUCCESS} Gobuster scan completed for {url}{AegisPalette.RESET}")
            ...
            logger.error(f"{AegisPalette.ERROR} Gobuster scan failed for {url}{AegisPalette.RESET}")

            # Check for alternative tool suggestion
            if result.get("alternative_tool_suggested"):
                alt_tool = result["alternative_tool_suggested"]
                logger.info(f"{AegisPalette.HIGHLIGHT_BLUE} Alternative tool suggested: {alt_tool} {AegisPalette.RESET}")
    ...
    def nuclei_scan(target: str, severity: str = "", tags: str = "", template: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Nuclei vulnerability scanner with structured logging and real-time progress.
        ...
        logger.info(f"{AegisPalette.BLOOD_RED} Starting Nuclei vulnerability scan: {target}{AegisPalette.RESET}")

        # Use enhanced error handling by default
        data["use_recovery"] = True
        result = relay_client.safe_post("api/tools/nuclei", data)

        if result.get("success"):
            logger.info(f"{AegisPalette.SUCCESS} Nuclei scan completed for {target}{AegisPalette.RESET}")

            # Enhanced vulnerability reporting
            if result.get("stdout") and "CRITICAL" in result["stdout"]:
                logger.warning(f"{AegisPalette.CRITICAL} CRITICAL vulnerabilities detected! {AegisPalette.RESET}")
            elif result.get("stdout") and "HIGH" in result["stdout"]:
                logger.warning(f"{AegisPalette.FIRE_RED} HIGH severity vulnerabilities found! {AegisPalette.RESET}")
            ...
            logger.error(f"{AegisPalette.ERROR} Nuclei scan failed for {target}{AegisPalette.RESET}")
    ...
    def prowler_scan(provider: str = "aws", profile: str = "default", region: str = "", checks: str = "", output_dir: str = "/tmp/prowler_output", output_format: str = "json", additional_args: str = "") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/tools/prowler", data)
        if result.get("success"):
    ...
    def trivy_scan(scan_type: str = "image", target: str = "", output_format: str = "json", severity: str = "", output_file: str = "", additional_args: str = "") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/tools/trivy", data)
        if result.get("success"):
    ...
    def scout_suite_assessment(provider: str = "aws", profile: str = "default",
        ...
        result = relay_client.safe_post("api/tools/scout-suite", data)
        if result.get("success"):
    ...
    def cloudmapper_analysis(action: str = "collect", account: str = "",
        ...
        result = relay_client.safe_post("api/tools/cloudmapper", data)
        if result.get("success"):
    ...
    @mcp.tool()
    def pacu_exploitation(session_name: str = "deepsec_session", modules: str = "",
        ...
        result = relay_client.safe_post("api/tools/pacu", data)
        if result.get("success"):
    ...
    def kube_hunter_scan(target: str = "", remote: str = "", cidr: str = "",
        ...
        result = relay_client.safe_post("api/tools/kube-hunter", data)
        if result.get("success"):
    ...
    def kube_bench_cis(targets: str = "", version: str = "", config_dir: str = "",
        ...
        result = relay_client.safe_post("api/tools/kube-bench", data)
        if result.get("success"):
    ...
    def docker_bench_security_scan(checks: str = "", exclude: str = "",
        ...
        result = relay_client.safe_post("api/tools/docker-bench-security", data)
        if result.get("success"):
    ...
    def clair_vulnerability_scan(image: str, config: str = "/etc/clair/config.yaml",
        ...
        result = relay_client.safe_post("api/tools/clair", data)
        if result.get("success"):
    ...
    def falco_runtime_monitoring(config_file: str = "/etc/falco/falco.yaml",
        ...
        result = relay_client.safe_post("api/tools/falco", data)
        if result.get("success"):
    ...
    def checkov_iac_scan(directory: str = ".", framework: str = "", check: str = "",
        ...
        result = relay_client.safe_post("api/tools/checkov", data)
        if result.get("success"):
    ...
    def terrascan_iac_scan(scan_type: str = "all", iac_dir: str = ".",
        ...
        result = relay_client.safe_post("api/tools/terrascan", data)
        if result.get("success"):
    ...
    def create_file(filename: str, content: str, binary: bool = False) -> Dict[str, Any]:
        """
        Create a file with specified content on the DeepSec server.
        ...
        result = relay_client.safe_post("api/files/create", data)
        if result.get("success"):
    ...
    def modify_file(filename: str, content: str, append: bool = False) -> Dict[str, Any]:
        """
        Modify an existing file on the DeepSec server.
        ...
        result = relay_client.safe_post("api/files/modify", data)
        if result.get("success"):
    ...
    def delete_file(filename: str) -> Dict[str, Any]:
        """
        Delete a file or directory on the DeepSec server.
        ...
        result = relay_client.safe_post("api/files/delete", data)
        if result.get("success"):
    ...
    def list_files(directory: str = ".") -> Dict[str, Any]:
        """
        List files in a directory on the DeepSec server.
        ...
        result = relay_client.safe_get("api/files/list", {"directory": directory})
        if result.get("success"):
    ...
    def generate_payload(payload_type: str = "buffer", size: int = 1024, pattern: str = "A", filename: str = "") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/payloads/generate", data)
        if result.get("success"):
    ...
    def install_python_package(package: str, env_name: str = "default") -> Dict[str, Any]:
        """
        Install a Python package in a virtual environment on the DeepSec server.
        ...
        result = relay_client.safe_post("api/python/install", data)
        if result.get("success"):
    ...
    def execute_python_script(script: str, env_name: str = "default", filename: str = "") -> Dict[str, Any]:
        """
        Execute a Python script in a virtual environment on the DeepSec server.
        ...
        result = relay_client.safe_post("api/python/execute", data)
        if result.get("success"):
    ...
    def dirb_scan(url: str, wordlist: str = "/usr/share/wordlists/dirb/common.txt", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Dirb for directory brute forcing with structured logging.
        ...
        result = relay_client.safe_post("api/tools/dirb", data)
        if result.get("success"):
    ...
    def nikto_scan(target: str, additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Nikto web vulnerability scanner with structured logging.
        ...
        result = relay_client.safe_post("api/tools/nikto", data)
        if result.get("success"):
    ...
    def sqlmap_scan(url: str, data: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute SQLMap for SQL injection testing with structured logging.
        ...
        result = relay_client.safe_post("api/tools/sqlmap", data_payload)
        if result.get("success"):
    ...
    def metasploit_run(module: str, options: Dict[str, Any] = {}) -> Dict[str, Any]:
        """
        Execute a Metasploit module with structured logging.
        ...
        result = relay_client.safe_post("api/tools/metasploit", data)
        if result.get("success"):
    ...
    def hydra_attack(
        ...
        """
        Execute Hydra for password brute forcing with structured logging.
        ...
        result = relay_client.safe_post("api/tools/hydra", data)
        if result.get("success"):
    ...
    def john_crack(
        ...
        """
        Execute John the Ripper for password cracking with structured logging.
        ...
        result = relay_client.safe_post("api/tools/john", data)
        if result.get("success"):
    ...
    def wpscan_analyze(url: str, additional_args: str = "") -> Dict[str, Any]:
        """
        Execute WPScan for WordPress vulnerability scanning with structured logging.
        ...
        result = relay_client.safe_post("api/tools/wpscan", data)
        if result.get("success"):
    ...
    def enum4linux_scan(target: str, additional_args: str = "-a") -> Dict[str, Any]:
        """
        Execute Enum4linux for SMB enumeration with structured logging.
        ...
        result = relay_client.safe_post("api/tools/enum4linux", data)
        if result.get("success"):
    ...
    def ffuf_scan(url: str, wordlist: str = "/usr/share/wordlists/dirb/common.txt", mode: str = "directory", match_codes: str = "200,204,301,302,307,401,403", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute FFuf for web fuzzing with structured logging.
        ...
        result = relay_client.safe_post("api/tools/ffuf", data)
        if result.get("success"):
    ...
    def netexec_scan(target: str, protocol: str = "smb", username: str = "", password: str = "", hash_value: str = "", module: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute NetExec (formerly CrackMapExec) for network enumeration with structured logging.
        ...
        result = relay_client.safe_post("api/tools/netexec", data)
        if result.get("success"):
    ...
    def amass_scan(domain: str, mode: str = "enum", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Amass for subdomain enumeration with structured logging.
        ...
        result = relay_client.safe_post("api/tools/amass", data)
        if result.get("success"):
    ...
    def hashcat_crack(hash_file: str, hash_type: str, attack_mode: str = "0", wordlist: str = "/usr/share/wordlists/rockyou.txt", mask: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Hashcat for advanced password cracking with structured logging.
        ...
        result = relay_client.safe_post("api/tools/hashcat", data)
        if result.get("success"):
    ...
    def subfinder_scan(domain: str, silent: bool = True, all_sources: bool = False, additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Subfinder for passive subdomain enumeration with structured logging.
        ...
        result = relay_client.safe_post("api/tools/subfinder", data)
        if result.get("success"):
    ...
    def smbmap_scan(target: str, username: str = "", password: str = "", domain: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute SMBMap for SMB share enumeration with structured logging.
        ...
        result = relay_client.safe_post("api/tools/smbmap", data)
        if result.get("success"):
    ...
    def rustscan_fast_scan(target: str, ports: str = "", ulimit: int = 5000,
        ...
        """
        Execute Rustscan for ultra-fast port scanning with structured logging.
        ...
        result = relay_client.safe_post("api/tools/rustscan", data)
        if result.get("success"):
    ...
    def masscan_high_speed(target: str, ports: str = "1-65535", rate: int = 1000,
        ...
        result = relay_client.safe_post("api/tools/masscan", data)
        if result.get("success"):
    ...
    def nmap_advanced_scan(target: str, scan_type: str = "-sS", ports: str = "",
        ...
        result = relay_client.safe_post("api/tools/nmap-advanced", data)
        if result.get("success"):
    ...
    def autorecon_comprehensive(target: str, output_dir: str = "/tmp/autorecon",
        ...
        result = relay_client.safe_post("api/tools/autorecon", data)
        if result.get("success"):
    ...
    def enum4linux_ng_advanced(target: str, username: str = "", password: str = "",
                               ...
                               additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Enum4linux-ng for advanced SMB enumeration with structured logging.
        ...
        result = relay_client.safe_post("api/tools/enum4linux-ng", data)
        if result.get("success"):
    ...
    def rpcclient_enumeration(target: str, username: str = "", password: str = "",
                             ...
                             additional_args: str = "") -> Dict[str, Any]:
        """
        Execute rpcclient for RPC enumeration with structured logging.
        ...
        result = relay_client.safe_post("api/tools/rpcclient", data)
        if result.get("success"):
    ...
    def nbtscan_netbios(target: str, verbose: bool = False, timeout: int = 2,
                       additional_args: str = "") -> Dict[str, Any]:
        """
        Execute nbtscan for NetBIOS name scanning with structured logging.
        ...
        result = relay_client.safe_post("api/tools/nbtscan", data)
        if result.get("success"):
    ...
    def arp_scan_discovery(target: str = "", interface: str = "", local_network: bool = False,
        ...
        """
        Execute arp-scan for network discovery with structured logging.
        ...
        result = relay_client.safe_post("api/tools/arp-scan", data)
        if result.get("success"):
    ...
    def responder_credential_harvest(interface: str = "eth0", analyze: bool = False,
                                   ...
                                   additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Responder for credential harvesting with structured logging.
        ...
        result = relay_client.safe_post("api/tools/responder", data)
        if result.get("success"):
    ...
    def volatility_analyze(memory_file: str, plugin: str, profile: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Volatility for memory forensics analysis with structured logging.
        ...
        result = relay_client.safe_post("api/tools/volatility", data)
        if result.get("success"):
    ...
    def msfvenom_generate(payload: str, format_type: str = "", output_file: str = "", encoder: str = "", iterations: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute MSFVenom for payload generation with structured logging.
        ...
        result = relay_client.safe_post("api/tools/msfvenom", data)
        if result.get("success"):
    ...
    def gdb_analyze(binary: str, commands: str = "", script_file: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute GDB for binary analysis and debugging with structured logging.
        ...
        result = relay_client.safe_post("api/tools/gdb", data)
        if result.get("success"):
    ...
    def radare2_analyze(binary: str, commands: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Radare2 for binary analysis and reverse engineering with structured logging.
        ...
        result = relay_client.safe_post("api/tools/radare2", data)
        if result.get("success"):
    ...
    def binwalk_analyze(file_path: str, extract: bool = False, additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Binwalk for firmware and file analysis with structured logging.
        ...
        result = relay_client.safe_post("api/tools/binwalk", data)
        if result.get("success"):
    ...
    def ropgadget_search(binary: str, gadget_type: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Search for ROP gadgets in a binary using ROPgadget with structured logging.
        ...
        result = relay_client.safe_post("api/tools/ropgadget", data)
        if result.get("success"):
    ...
    def checksec_analyze(binary: str) -> Dict[str, Any]:
        """
        Check security features of a binary with structured logging.
        ...
        result = relay_client.safe_post("api/tools/checksec", data)
        if result.get("success"):
    ...
    def xxd_hexdump(file_path: str, offset: str = "0", length: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Create a hex dump of a file using xxd with structured logging.
        ...
        result = relay_client.safe_post("api/tools/xxd", data)
        if result.get("success"):
    ...
    def strings_extract(file_path: str, min_len: int = 4, additional_args: str = "") -> Dict[str, Any]:
        """
        Extract strings from a binary file with structured logging.
        ...
        result = relay_client.safe_post("api/tools/strings", data)
        if result.get("success"):
    ...
    def objdump_analyze(binary: str, disassemble: bool = True, additional_args: str = "") -> Dict[str, Any]:
        """
        Analyze a binary using objdump with structured logging.
        ...
        result = relay_client.safe_post("api/tools/objdump", data)
        if result.get("success"):
    ...
    @mcp.tool()
    def ghidra_analysis(binary: str, project_name: str = "deepsec_analysis",
        ...
        result = relay_client.safe_post("api/tools/ghidra", data)
        if result.get("success"):
    ...
    def pwntools_exploit(script_content: str = "", target_binary: str = "",
        ...
        result = relay_client.safe_post("api/tools/pwntools", data)
        if result.get("success"):
    ...
    def one_gadget_search(libc_path: str, level: int = 1, additional_args: str = "") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/tools/one-gadget", data)
        if result.get("success"):
    ...
    def libc_database_lookup(action: str = "find", symbols: str = "",
        ...
        result = relay_client.safe_post("api/tools/libc-database", data)
        if result.get("success"):
    ...
    def gdb_peda_debug(binary: str = "", commands: str = "", attach_pid: int = 0,
        ...
        result = relay_client.safe_post("api/tools/gdb-peda", data)
        if result.get("success"):
    ...
    def angr_symbolic_execution(binary: str, script_content: str = "",
        ...
        result = relay_client.safe_post("api/tools/angr", data)
        if result.get("success"):
    ...
    def ropper_gadget_search(binary: str, gadget_type: str = "rop", quality: int = 1,
        ...
        result = relay_client.safe_post("api/tools/ropper", data)
        if result.get("success"):
    ...
    def pwninit_setup(binary: str, libc: str = "", ld: str = "",
        ...
        result = relay_client.safe_post("api/tools/pwninit", data)
        if result.get("success"):
    ...
    def feroxbuster_scan(url: str, wordlist: str = "/usr/share/wordlists/dirb/common.txt", threads: int = 10, additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Feroxbuster for recursive content discovery with structured logging.
        ...
        result = relay_client.safe_post("api/tools/feroxbuster", data)
        if result.get("success"):
    ...
    def dotdotpwn_scan(target: str, module: str = "http", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute DotDotPwn for directory traversal testing with structured logging.
        ...
        result = relay_client.safe_post("api/tools/dotdotpwn", data)
        if result.get("success"):
    ...
    def xsser_scan(url: str, params: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute XSSer for XSS vulnerability testing with structured logging.
        ...
        result = relay_client.safe_post("api/tools/xsser", data)
        if result.get("success"):
    ...
    def wfuzz_scan(url: str, wordlist: str = "/usr/share/wordlists/dirb/common.txt", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Wfuzz for web application fuzzing with structured logging.
        ...
        result = relay_client.safe_post("api/tools/wfuzz", data)
        if result.get("success"):
    ...
    def dirsearch_scan(url: str, extensions: str = "php,html,js,txt,xml,json",
        ...
        """
        Execute Dirsearch for advanced directory and file discovery with structured logging.
        ...
        result = relay_client.safe_post("api/tools/dirsearch", data)
        if result.get("success"):
    ...
    def katana_crawl(url: str, depth: int = 3, js_crawl: bool = True,
                    ...
                    additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Katana for next-generation crawling and spidering with structured logging.
        ...
        result = relay_client.safe_post("api/tools/katana", data)
        if result.get("success"):
    ...
    def gau_discovery(domain: str, providers: str = "wayback,commoncrawl,otx,urlscan",
                     ...
                     additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Gau (Get All URLs) for URL discovery from multiple sources with structured logging.
        ...
        result = relay_client.safe_post("api/tools/gau", data)
        if result.get("success"):
    ...
    def waybackurls_discovery(domain: str, get_versions: bool = False,
        ...
        """
        Execute Waybackurls for historical URL discovery with structured logging.
        ...
        result = relay_client.safe_post("api/tools/waybackurls", data)
        if result.get("success"):
    ...
    def arjun_parameter_discovery(url: str, method: str = "GET", wordlist: str = "",
                                 ...
                                 additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Arjun for HTTP parameter discovery with structured logging.
        ...
        result = relay_client.safe_post("api/tools/arjun", data)
        if result.get("success"):
    ...
    def paramspider_mining(domain: str, level: int = 2,
        ...
        """
        Execute ParamSpider for parameter mining from web archives with structured logging.
        ...
        result = relay_client.safe_post("api/tools/paramspider", data)
        if result.get("success"):
    ...
    def x8_parameter_discovery(url: str, wordlist: str = "/usr/share/wordlists/x8/params.txt",
                              ...
                              additional_args: str = "") -> Dict[str, Any]:
        """
        Execute x8 for hidden parameter discovery with structured logging.
        ...
        result = relay_client.safe_post("api/tools/x8", data)
        if result.get("success"):
    ...
    def jaeles_vulnerability_scan(url: str, signatures: str = "", config: str = "",
        ...
        result = relay_client.safe_post("api/tools/jaeles", data)
        if result.get("success"):
    ...
    def dalfox_xss_scan(url: str, pipe_mode: bool = False, blind: bool = False,
        ...
        """
        Execute Dalfox for advanced XSS vulnerability scanning with structured logging.
        ...
        result = relay_client.safe_post("api/tools/dalfox", data)
        if result.get("success"):
    ...
    def httpx_probe(target: str, probe: bool = True, tech_detect: bool = False,
        ...
        result = relay_client.safe_post("api/tools/httpx", data)
        if result.get("success"):
    ...
    def anew_data_processing(input_data: str, output_file: str = "",
        ...
        result = relay_client.safe_post("api/tools/anew", data)
        if result.get("success"):
    ...
    def qsreplace_parameter_replacement(urls: str, replacement: str = "FUZZ",
        ...
        result = relay_client.safe_post("api/tools/qsreplace", data)
        if result.get("success"):
    ...
    def uro_url_filtering(urls: str, whitelist: str = "", blacklist: str = "",
        ...
        result = relay_client.safe_post("api/tools/uro", data)
        if result.get("success"):
    ...
    def ai_generate_payload(attack_type: str, complexity: str = "basic", technology: str = "", url: str = "") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/ai/generate_payload", data)
    ...
    def ai_test_payload(payload: str, target_url: str, method: str = "GET") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/ai/test_payload", data)
    ...
    def ai_runtime_health() -> Dict[str, Any]:
        """
        Check centralized AI runtime health and Ollama model availability.

        Returns:
            Runtime status including configured model readiness
        """
        logger.info(" Checking AI runtime health")
        result = relay_client.safe_get("api/ai/health")

        if result.get("success"):
            model = result.get("model", "unknown")
            runtime = result.get("ollama", {})
            logger.info(f" AI runtime online | Model: {model} | Ready: {runtime.get('model_ready', False)}")
        else:
            logger.error(" AI runtime health check failed")

        return result

    @mcp.tool()
    def ai_chat(message: str, context: str = "{}") -> Dict[str, Any]:
        """
        Send an operator message to the centralized AI runtime.

        Args:
            message: Prompt for AI runtime
            context: JSON string with optional operational context

        Returns:
            AI response and runtime metadata
        """
        import json
        ...
        try:
            context_data = json.loads(context) if context and context != "{}" else {}
        except Exception:
            context_data = {}

        logger.info(" Sending message to AI runtime")
        result = relay_client.safe_post("api/ai/chat", {"message": message, "context": context_data})

        if result.get("success"):
            logger.info(" AI runtime chat completed")
        else:
            logger.error(" AI runtime chat failed")
    ...
    def api_fuzzer(base_url: str, endpoints: str = "", methods: str = "GET,POST,PUT,DELETE", wordlist: str = "/usr/share/wordlists/api/api-endpoints.txt") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/tools/api_fuzzer", data)
    ...
    def graphql_scanner(endpoint: str, introspection: bool = True, query_depth: int = 10, test_mutations: bool = True) -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/tools/graphql_scanner", data)
    ...
    def jwt_analyzer(jwt_token: str, target_url: str = "") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/tools/jwt_analyzer", data)
    ...
    def api_schema_analyzer(schema_url: str, schema_type: str = "openapi") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/tools/api_schema_analyzer", data)
    ...
    def volatility3_analyze(memory_file: str, plugin: str, output_file: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Volatility3 for advanced memory forensics with structured logging.
        ...
        result = relay_client.safe_post("api/tools/volatility3", data)
        if result.get("success"):
    ...
    def foremost_carving(input_file: str, output_dir: str = "/tmp/foremost_output", file_types: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Foremost for file carving with structured logging.
        ...
        result = relay_client.safe_post("api/tools/foremost", data)
        if result.get("success"):
    ...
    def steghide_analysis(action: str, cover_file: str, embed_file: str = "", passphrase: str = "", output_file: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Steghide for steganography analysis with structured logging.
        ...
        result = relay_client.safe_post("api/tools/steghide", data)
        if result.get("success"):
    ...
    def exiftool_extract(file_path: str, output_format: str = "", tags: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute ExifTool for metadata extraction with structured logging.
        ...
        result = relay_client.safe_post("api/tools/exiftool", data)
        if result.get("success"):
    ...
    def hashpump_attack(signature: str, data: str, key_length: str, append_data: str, additional_args: str = "") -> Dict[str, Any]:
        """
        Execute HashPump for hash length extension attacks with structured logging.
        ...
        result = relay_client.safe_post("api/tools/hashpump", data)
        if result.get("success"):
    ...
    def hakrawler_crawl(url: str, depth: int = 2, forms: bool = True, robots: bool = True, sitemap: bool = True, wayback: bool = False, additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Hakrawler for web endpoint discovery with structured logging.
        ...
        result = relay_client.safe_post("api/tools/hakrawler", data)
        if result.get("success"):
    ...
    def httpx_probe(targets: str = "", target_file: str = "", ports: str = "", methods: str = "GET", status_code: str = "", content_length: bool = False, output_file: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute HTTPx for HTTP probing with structured logging.
    ...
    def paramspider_discovery(domain: str, exclude: str = "", output_file: str = "", level: int = 2, additional_args: str = "") -> Dict[str, Any]:
        """
        Execute ParamSpider for parameter discovery with structured logging.
    ...
    def burpsuite_scan(project_file: str = "", config_file: str = "", target: str = "", headless: bool = False, scan_type: str = "", scan_config: str = "", output_file: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Burp Suite with structured logging.
        ...
        result = relay_client.safe_post("api/tools/burpsuite", data)
        if result.get("success"):
    ...
    def zap_scan(target: str = "", scan_type: str = "baseline", api_key: str = "", daemon: bool = False, port: str = "8090", host: str = "0.0.0.0", format_type: str = "xml", output_file: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute OWASP ZAP with structured logging.
        ...
        result = relay_client.safe_post("api/tools/zap", data)
        if result.get("success"):
    ...
    def arjun_scan(url: str, method: str = "GET", data: str = "", headers: str = "", timeout: str = "", output_file: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Arjun for parameter discovery with structured logging.
    ...
    def wafw00f_scan(target: str, additional_args: str = "") -> Dict[str, Any]:
        """
        Execute wafw00f to identify and fingerprint WAF products with structured logging.
        ...
        result = relay_client.safe_post("api/tools/wafw00f", data)
        if result.get("success"):
    ...
    def fierce_scan(domain: str, dns_server: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute fierce for DNS reconnaissance with structured logging.
        ...
        result = relay_client.safe_post("api/tools/fierce", data)
        if result.get("success"):
    ...
    def dnsenum_scan(domain: str, dns_server: str = "", wordlist: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute dnsenum for DNS enumeration with structured logging.
        ...
        result = relay_client.safe_post("api/tools/dnsenum", data)
        if result.get("success"):
    ...
    def server_health() -> Dict[str, Any]:
        """
        Check the health status of the DeepSec Control Plane server.

        Returns:
            Server health information with tool availability and telemetry
        """
        logger.info(f" Checking DeepSec Control Plane server health")
        result = relay_client.check_health()
    ...
    def get_cache_stats() -> Dict[str, Any]:
        """
        Get cache statistics from the DeepSec Control Plane server.

        Returns:
            Cache performance statistics
        """
        logger.info(f" Getting cache statistics")
        result = relay_client.safe_get("api/cache/stats")
    ...
    def clear_cache() -> Dict[str, Any]:
        """
        Clear the cache on the DeepSec Control Plane server.

        Returns:
            Cache clear operation results
        """
        logger.info(f" Clearing server cache")
        result = relay_client.safe_post("api/cache/clear", {})
        if result.get("success"):
    ...
    def get_telemetry() -> Dict[str, Any]:
        """
        Get system telemetry from the DeepSec Control Plane server.

        Returns:
            System performance and usage telemetry
        """
        logger.info(f" Getting system telemetry")
        result = relay_client.safe_get("api/telemetry")
    ...
    def list_active_processes() -> Dict[str, Any]:
        """
        List all active processes on the DeepSec Control Plane server.

        Returns:
            List of active processes with their status and progress
        """
        logger.info(" Listing active processes")
        result = relay_client.safe_get("api/processes/list")
        if result.get("success"):
    ...
    def get_process_status(pid: int) -> Dict[str, Any]:
        ...
        result = relay_client.safe_get(f"api/processes/status/{pid}")
        if result.get("success"):
    ...
    def terminate_process(pid: int) -> Dict[str, Any]:
        ...
        result = relay_client.safe_post(f"api/processes/terminate/{pid}", {})
        if result.get("success"):
    ...
    def pause_process(pid: int) -> Dict[str, Any]:
        ...
        result = relay_client.safe_post(f"api/processes/pause/{pid}", {})
        if result.get("success"):
    ...
    def resume_process(pid: int) -> Dict[str, Any]:
        ...
        result = relay_client.safe_post(f"api/processes/resume/{pid}", {})
        if result.get("success"):
    ...
    def get_process_dashboard() -> Dict[str, Any]:
        ...
        result = relay_client.safe_get("api/processes/dashboard")
    ...
    def execute_command(command: str, use_cache: bool = True) -> Dict[str, Any]:
        """
        Execute an arbitrary command on the DeepSec Control Plane server with structured logging.
        ...
        try:
            ...
            result = relay_client.execute_command(command, use_cache)
            if "error" in result:
    ...
    def monitor_cve_feeds(hours: int = 24, severity_filter: str = "HIGH,CRITICAL", keywords: str = "") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/vuln-intel/cve-monitor", data)
    ...
    def generate_exploit_from_cve(cve_id: str, target_os: str = "", target_arch: str = "x64", exploit_type: str = "poc", evasion_level: str = "none") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/vuln-intel/exploit-generate", data)
    ...
    def discover_attack_chains(target_software: str, attack_depth: int = 3, include_zero_days: bool = False) -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/vuln-intel/attack-chains", data)
    ...
    def research_zero_day_opportunities(target_software: str, analysis_depth: str = "standard", source_code_url: str = "") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/vuln-intel/zero-day-research", data)
    ...
    def correlate_threat_intelligence(indicators: str, timeframe: str = "30d", sources: str = "all") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/vuln-intel/threat-feeds", data)
    ...
    def advanced_payload_generation(attack_type: str, target_context: str = "", evasion_level: str = "standard", custom_constraints: str = "") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/ai/advanced-payload-generation", data)
    ...
    def vulnerability_intelligence_dashboard() -> Dict[str, Any]:
        ...
        latest_cves = relay_client.safe_post("api/vuln-intel/cve-monitor", {
            "hours": 24,
        ...
        trending_research = relay_client.safe_post("api/vuln-intel/zero-day-research", {
    ...
    def get_live_dashboard() -> Dict[str, Any]:
        """
        Get a structured live dashboard showing active processes with terminal-friendly formatting.
    ...
    def create_vulnerability_report(vulnerabilities: str, target: str = "", scan_type: str = "comprehensive") -> Dict[str, Any]:
        """
        Create a structured vulnerability report with severity-based styling and visual indicators.
        ...
        try:
            ...
            for vuln in vuln_data:
                card_result = relay_client.safe_post("api/visual/vulnerability-card", vuln)
            ...
            summary_result = relay_client.safe_post("api/visual/summary-report", summary_data)
    ...
    def format_tool_output_visual(tool_name: str, output: str, success: bool = True) -> Dict[str, Any]:
        """
        Format tool output with structured terminal styling and clear layout.
        ...
        Returns:
            Formatted tool output with structured presentation
        """
        ...
        result = relay_client.safe_post("api/visual/tool-output", data)
        if result.get("success"):
    ...
    def create_scan_summary(target: str, tools_used: str, vulnerabilities_found: int = 0,
        ...
        """
        Create a comprehensive scan summary report with structured terminal formatting.
        ...
        Returns:
            Structured scan summary report
        """
        ...
        result = relay_client.safe_post("api/visual/summary-report", summary_data)
        if result.get("success"):
    ...
    def display_system_metrics() -> Dict[str, Any]:
        """
        Display current system metrics and performance indicators with structured formatting.

        Returns:
            System metrics with structured presentation
        """
        logger.info(" Fetching system metrics")

        # Get telemetry data
        telemetry_result = relay_client.safe_get("api/telemetry")
...
""",
    ...
    def analyze_target_intelligence(target: str) -> Dict[str, Any]:
        ...
        data = {"target": target}
        result = relay_client.safe_post("api/intelligence/analyze-target", data)
    ...
    def select_optimal_tools_ai(target: str, objective: str = "comprehensive") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/intelligence/select-tools", data)
    ...
    def optimize_tool_parameters_ai(target: str, tool: str, context: str = "{}") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/intelligence/optimize-parameters", data)
    ...
    def create_attack_chain_ai(target: str, objective: str = "comprehensive") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/intelligence/create-attack-chain", data)
    ...
    def intelligent_smart_scan(target: str, objective: str = "comprehensive", max_tools: int = 5) -> Dict[str, Any]:
        ...
        """
        logger.info(f"{AegisPalette.FIRE_RED} Starting intelligent smart scan for {target}{AegisPalette.RESET}")
        ...
        result = relay_client.safe_post("api/intelligence/smart-scan", data)
        ...
        if result.get("success"):
            ...
            # Structured logging with detailed results
            logger.info(f"{AegisPalette.SUCCESS} Intelligent scan completed for {target}{AegisPalette.RESET}")
            logger.info(f"{AegisPalette.CYBER_ORANGE} Execution Summary:{AegisPalette.RESET}")
            ...
            if successful_tools:
                logger.info(f"{AegisPalette.HIGHLIGHT_GREEN} Successful tools: {', '.join(successful_tools)} {AegisPalette.RESET}")

            # Log failed tools
            failed_tools = [t['tool'] for t in tools_executed if not t.get('success')]
            if failed_tools:
                logger.warning(f"{AegisPalette.HIGHLIGHT_RED} Failed tools: {', '.join(failed_tools)} {AegisPalette.RESET}")

            # Log vulnerabilities found
            if scan_results.get('total_vulnerabilities', 0) > 0:
                logger.warning(f"{AegisPalette.VULN_HIGH} {scan_results['total_vulnerabilities']} vulnerabilities detected!{AegisPalette.RESET}")
        else:
            logger.error(f"{AegisPalette.ERROR} Intelligent scan failed for {target}: {result.get('error', 'Unknown error')}{AegisPalette.RESET}")
    ...
    def detect_technologies_ai(target: str) -> Dict[str, Any]:
        ...
        data = {"target": target}
        result = relay_client.safe_post("api/intelligence/technology-detection", data)
    ...
    def ai_reconnaissance_workflow(target: str, depth: str = "standard") -> Dict[str, Any]:
        ...
        analysis_result = relay_client.safe_post("api/intelligence/analyze-target", {"target": target})
        ...
        chain_result = relay_client.safe_post("api/intelligence/create-attack-chain", {
            "target": target,
            "objective": objective
        })
        ...
        scan_result = relay_client.safe_post("api/intelligence/smart-scan", {
            "target": target,
            "objective": objective,
    ...
    def bugbounty_reconnaissance_workflow(domain: str, scope: str = "", out_of_scope: str = "",
        ...
        result = relay_client.safe_post("api/bugbounty/reconnaissance-workflow", data)
    ...
    def bugbounty_vulnerability_hunting(domain: str, priority_vulns: str = "rce,sqli,xss,idor,ssrf",
        ...
        result = relay_client.safe_post("api/bugbounty/vulnerability-hunting-workflow", data)
    ...
    def bugbounty_business_logic_testing(domain: str, program_type: str = "web") -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/bugbounty/business-logic-workflow", data)
    ...
    def bugbounty_osint_gathering(domain: str) -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/bugbounty/osint-workflow", data)
    ...
    def bugbounty_file_upload_testing(target_url: str) -> Dict[str, Any]:
        ...
        result = relay_client.safe_post("api/bugbounty/file-upload-testing", data)
    ...
    def bugbounty_comprehensive_assessment(domain: str, scope: str = "",
        ...
        result = relay_client.safe_post("api/bugbounty/comprehensive-assessment", data)
    ...
    def http_framework_test(url: str, method: str = "GET", data: dict = {},
        ...
        logger.info(f"{AegisPalette.FIRE_RED} Starting HTTP Framework {action}: {url}{AegisPalette.RESET}")
        result = relay_client.safe_post("api/tools/http-framework", data_payload)

        if result.get("success"):
            logger.info(f"{AegisPalette.SUCCESS} HTTP Framework {action} completed for {url}{AegisPalette.RESET}")

            # Structured logging for vulnerabilities found
            if result.get("result", {}).get("vulnerabilities"):
                vuln_count = len(result["result"]["vulnerabilities"])
                logger.info(f"{AegisPalette.HIGHLIGHT_RED} Found {vuln_count} potential vulnerabilities {AegisPalette.RESET}")
        else:
            logger.error(f"{AegisPalette.ERROR} HTTP Framework {action} failed for {url}{AegisPalette.RESET}")
    ...
    def browser_agent_inspect(url: str, headless: bool = True, wait_time: int = 5,
        ...
        logger.info(f"{AegisPalette.CRIMSON} Starting Browser Agent {action}: {url}{AegisPalette.RESET}")
        result = relay_client.safe_post("api/tools/browser-agent", data_payload)

        if result.get("success"):
            logger.info(f"{AegisPalette.SUCCESS} Browser Agent {action} completed for {url}{AegisPalette.RESET}")
            ...
            if action == "navigate" and result.get("result", {}).get("security_analysis"):
                ...
                if issues_count > 0:
                    logger.warning(f"{AegisPalette.HIGHLIGHT_YELLOW} Security Issues: {issues_count} | Score: {security_score}/100 {AegisPalette.RESET}")
                else:
                    logger.info(f"{AegisPalette.HIGHLIGHT_GREEN} No security issues found | Score: {security_score}/100 {AegisPalette.RESET}")
        else:
            logger.error(f"{AegisPalette.ERROR} Browser Agent {action} failed for {url}{AegisPalette.RESET}")
    ...
    def http_set_rules(rules: list) -> Dict[str, Any]:
        ...
        return relay_client.safe_post("api/tools/http-framework", payload)
    ...
    def burpsuite_alternative_scan(target: str, scan_type: str = "comprehensive",
        ...
        logger.info(f"{AegisPalette.BLOOD_RED} Starting Burp Suite Alternative {scan_type} scan: {target}{AegisPalette.RESET}")
        result = relay_client.safe_post("api/tools/burpsuite-alternative", data_payload)

        if result.get("success"):
            logger.info(f"{AegisPalette.SUCCESS} Burp Suite Alternative scan completed for {target}{AegisPalette.RESET}")

            # Structured logging for comprehensive results
            if result.get("result", {}).get("summary"):
                ...
                logger.info(f"{AegisPalette.HIGHLIGHT_BLUE} SCAN SUMMARY {AegisPalette.RESET}")
                ...
                for severity, count in vuln_breakdown.items():
                    if count > 0:
                        color = {
                                    'critical': AegisPalette.CRITICAL,
        'high': AegisPalette.FIRE_RED,
        'medium': AegisPalette.CYBER_ORANGE,
        'low': AegisPalette.YELLOW,
        'info': AegisPalette.INFO
    }.get(severity.lower(), AegisPalette.WHITE)

                        logger.info(f"  {color}{severity.upper()}: {count}{AegisPalette.RESET}")
        else:
            logger.error(f"{AegisPalette.ERROR} Burp Suite Alternative scan failed for {target}{AegisPalette.RESET}")
    ...
    def error_handling_statistics() -> Dict[str, Any]:
        ...
        """
        logger.info(f"{AegisPalette.ELECTRIC_PURPLE} Retrieving error handling statistics{AegisPalette.RESET}")
        result = relay_client.safe_get("api/error-handling/statistics")
        ...
        if result.get("success"):
            ...
            logger.info(f"{AegisPalette.SUCCESS} Error statistics retrieved{AegisPalette.RESET}")
            ...
            if error_counts:
                logger.info(f"{AegisPalette.HIGHLIGHT_BLUE} ERROR BREAKDOWN {AegisPalette.RESET}")
                for error_type, count in error_counts.items():
                                          logger.info(f"  {AegisPalette.FIRE_RED}{error_type}: {count}{AegisPalette.RESET}")
        else:
            logger.error(f"{AegisPalette.ERROR} Failed to retrieve error statistics{AegisPalette.RESET}")
    ...
    def test_error_recovery(tool_name: str, error_type: str = "timeout",
        ...
        logger.info(f"{AegisPalette.RUBY} Testing error recovery for {tool_name} with {error_type}{AegisPalette.RESET}")
        result = relay_client.safe_post("api/error-handling/test-recovery", data_payload)
        ...
        if result.get("success"):
            ...
            logger.info(f"{AegisPalette.SUCCESS} Error recovery test completed{AegisPalette.RESET}")
            ...
            logger.error(f"{AegisPalette.ERROR} Error recovery test failed{AegisPalette.RESET}")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Run the DeepSec MCP Client")
    parser.add_argument("--server", type=str, default=DEFAULT_CONTROL_PLANE_URL,
                      help=f"DeepSec Control Plane API server URL (default: {DEFAULT_CONTROL_PLANE_URL})")
...
def main():
    ...
    logger.info(f" Starting DeepSec MCP Client v6.0")
    logger.info(f" Connecting to: {args.server}")

    try:
        # Initialize the DeepSec Control Plane client
        relay_client = RelayOpsClient(args.server, args.timeout)

        # Check server health and log the result
        health = relay_client.check_health()
        if "error" in health:
            logger.warning(f"  Unable to connect to DeepSec Control Plane API server at {args.server}: {health['error']}")
            logger.warning(" MCP server will start, but tool execution may fail")
        else:
            logger.info(f" Successfully connected to DeepSec Control Plane API server at {args.server}")
            logger.info(f" Server health status: {health['status']}")
            logger.info(f" Version: {health.get('version', 'unknown')}")
            if not health.get("all_essential_tools_available", False):
                logger.warning("  Not all essential tools are available on the DeepSec Control Plane")
                missing_tools = [tool for tool, available in health.get("tools_status", {}).items() if not available]
                if missing_tools:
                    logger.warning(f" Missing tools: {', '.join(missing_tools[:5])}{'...' if len(missing_tools) > 5 else ''}")

        # Set up and run the MCP server
        mcp = assemble_tool_mesh(relay_client)
        logger.info(" Starting DeepSec MCP server")
